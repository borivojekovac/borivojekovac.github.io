...
(function (root) {
  ...
  P.review = ({ pid, run: id }) => withP(pid, (p) => {
    ...
    // Same sections as the work item's live document (owner review round 11): heading without a count, a short status
    // after it while collapsed, no rules between sections.
    const sec = (k, title, body, open, preview = '') => `<details class="doc-section review-section" data-section="${k}" ${open ? 'open' : ''}><summary>${icon('expand_more')}<span class="doc-section__title">${esc(title)}</span>${preview ? `<span class="doc-section__preview">${esc(preview)}</span>` : ''}</summary><div class="doc-section__body">${body}</div></details>`;
    const n = (x, one, many = `${one}s`, none = `No ${many}`) => (x ? `${x} ${x === 1 ? one : many}` : none);
    ...
    const steps = ADE.runSteps(p, r); const stepsDone = steps.filter((x) => x.status === 'done').length;
    const testsOk = testRuns.length && testRuns[testRuns.length - 1].outcome === 'pass'; const met = w.criteria.length + (testsOk ? 1 : 0); const checksTotal = w.criteria.length + 2;
    const adds = r.files.reduce((t, x) => t + x.add, 0); const dels = r.files.reduce((t, x) => t + x.del, 0);
    return { eyebrow: ['visibility', `Review · run ${id.replace('RUN-', '')} · attempt ${r.attempt}`], title: w.title, wide: true,
      body: `${sec('summary', 'Summary', `<p>${esc(ADE.profile(p, r.profile) ? ADE.profile(p, r.profile).title : r.profile)} produced ${r.files.length} changed files${arts.length ? ` and ${arts.join(', ')}` : ''} for ${esc(w.id)}. ${testRuns.length ? `Tests ran ${testRuns.length}× (last: ${esc(testRuns[testRuns.length - 1].outcome)}).` : ''} Spent ${esc(eur(r.spent))} of ${esc(eur(r.reserved))}.</p><p class="meta">${esc(r.policy)}${r.prev ? ` · follows ${esc(r.prev)}` : ''}</p><p class="meta">Accepting marks the work item's contribution done. It does not accept the release or the engineering version.</p>`, true, `${n(r.files.length, 'file')} · ${eur(r.spent)} of ${eur(r.reserved)}`)}
        ${sec('steps', 'Workflow steps', ADE.stepLine(p, r), false, `${stepsDone} of ${steps.length} done`)}
        ${sec('artifacts', 'Artifacts', artBody, false, arts.length ? arts.join(', ') : 'None')}
        ${sec('files', 'Changed files', r.files.length ? ADE.changedFiles(p, r, 'review') : '<p class="meta">No files changed (read-only profile).</p>', false, r.files.length ? `${n(r.files.length, 'file')} · +${adds} −${dels}` : 'None')}
        ${sec('criteria', 'Acceptance criteria and required checks', `<ul class="checks">${w.criteria.map((c) => `<li class="is-ok">${icon('check_circle')}<span>${esc(c.id)} ${esc(c.text)}<small>Evidence: covered by ${esc((r.files.find((f) => /test/.test(f.path)) || { path: 'the submitted change' }).path)} (synthetic)</small></span></li>`).join('')}<li class="${testRuns.length && testRuns[testRuns.length - 1].outcome === 'pass' ? 'is-ok' : 'is-missing'}">${icon(testRuns.length && testRuns[testRuns.length - 1].outcome === 'pass' ? 'check_circle' : 'radio_button_unchecked')}<span>Tests pass<small>${testRuns.length ? `Test step: ${testRuns.map((t) => t.outcome).join(' → ')}` : 'No test step in this profile'}</small></span></li><li class="is-missing">${icon('radio_button_unchecked')}<span>Human review (policy)<small>This review</small></span></li></ul>`, false, `${met} of ${checksTotal} met`)}
        ${sec('risks', 'Risks raised or linked', risks.length ? risks.map((x) => `<p>${ADE.badge(x.status)} ${ADE.a(`${x.id} ${x.title}`, ADE.plink(pid, 'risks', { risk: x.id }))}</p>`).join('') : '<p class="meta">No open risks on this item.</p>', false, n(risks.length, 'open risk', 'open risks', 'None open'))}
        ${sec('comments', 'Comments', (w.comments || []).length ? `<div class="comments">${w.comments.map((c) => `<div class="comment${c.agent ? ' comment--agent' : ''}">${c.agent ? '<span class="avatar">AI</span>' : `<span class="avatar avatar--${esc(c.by)}">${esc((F.people[c.by] || {}).initials || '?')}</span>`}<div><p class="meta"><strong>${esc(c.agent ? 'Agent' : ADE.personName(c.by))}</strong>${c.at ? ` · ${esc(c.at)}` : ''}</p><p>${esc(c.text)}</p></div></div>`).join('')}</div>` : '<p class="meta">No comments.</p>', false, n((w.comments || []).length, 'comment'))}
