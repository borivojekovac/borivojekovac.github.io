...
(function (root) {
  ...
  scr.dashboard = function (p) {
    ...
    return `${ADE.pageHeader({
      ...
      facts: [['Description', `<span class="fact-edit"><span class="clamp">${esc(p.description)}</span>${ADE.editBtn(ADE.can(p, 'description'), 'descriptionOpen', { pid: p.id }, 'project description')}</span>`], ['Budget', `${ADE.costRisk(t, p.budget.amount)}`], ['Time', ADE.timeRisk({ timeRatio: p.budget.timeRatio, planned: p.budget.planned })], ['Policy', `<a class="link" href="${esc(ADE.plink(p.id, 'config', { contract: 'policy' }))}">Revision ${p.contracts.policy.revision}</a>`], ['Agents', `${ADE.capacity(p).used} of ${ADE.capacity(p).slots} slots busy`]],
    })}
    ${welcome}
    <div class="dash">
      ...
      <aside class="dash__side">
        ${section('Cost and time', `<div class="card cost">
          ...
          ${p.releases.map((r) => { const rt = ADE.releaseTotals(p, r); return `<div class="cost-row"><div class="cost-row__head">${ADE.a(r.name, ADE.plink(p.id, `releases/${r.id}`))}${ADE.costRisk(rt, r.budget)}</div>${ADE.costGauge(rt, r.budget, true)}<div class="cost-row__time">${ADE.timeRisk(r)}</div></div>`; }).join('')}`)}
  ...
  const docSection = (title, body, open = true, preview = '', edit = '') => `<details class="doc-section" ${open ? 'open' : ''}><summary>${icon('expand_more')}<span class="doc-section__title">${esc(title)}</span>${edit}<span class="doc-section__preview">${esc(preview)}</span></summary><div class="doc-section__body">${body}</div></details>`;
  ...
  scr.workItem = function (p, wid) {
    ...
    if (open && leaf && !it.decisionItem) {
      ...
      else if (rd.state === 'ready') primary = ev.verdict === 'deny' ? `<button class="btn btn--primary" type="button" disabled>${icon('block')}Start agent</button><span class="meta">${esc(ev.reason)}</span>` : ADE.btn(ADE.startLabel(ev), 'startOpen', { pid: p.id, wid: it.id }, 'primary', 'smart_toy');
      else if (onlyConfirm) primary = ADE.gbtn(G('mark-ready'), 'Mark ready', 'markReady', { pid: p.id, wid: it.id }, 'primary', 'check_circle');
      else primary = `<button class="btn btn--primary" type="button" disabled>${icon('smart_toy')}Start agent</button><span class="meta">${esc(failing[0] ? failing[0].fix : ADE.label(rd.state))}</span>`;
    }
    const doneGrant = G('mark-done'); const cc = ADE.canComplete(p, it);
    const markDone = open && !run ? (cc.ok ? ADE.gbtn(doneGrant, leaf ? 'Record human contribution' : 'Mark done', 'doneOpen', { pid: p.id, wid: it.id }, 'quiet', 'task_alt') : `<span class="granted-off"><button class="btn btn--quiet" type="button" disabled aria-describedby="why-done-${esc(it.id)}">${icon('task_alt')}Mark done</button><small class="granted-off__why" id="why-done-${esc(it.id)}" data-for="Mark done">${icon('info')}${esc(cc.why)}</small></span>`) : '';
    const sibs = it.parentId ? ADE.kids(p, it.parentId) : ADE.roots(p); const si = sibs.indexOf(it);
    const actions = `${primary}${it.status !== 'done' ? (G('split').ok ? ADE.a('Split…', ADE.plink(p.id, `work/${it.id}/split`), 'btn btn--outline') : ADE.gbtn(G('split'), 'Split…', 'noop', { wid: it.id }, 'outline')) : ''}${ADE.gbtn(G('add-child'), 'Add child', 'newWorkOpen', { pid: p.id, parent: it.id }, 'quiet', 'add')}${ADE.gbtn(ADE.can(p, 'request-contribution'), 'Request contribution', 'contributionOpen', { pid: p.id, wid: it.id }, 'quiet', 'forward_to_inbox')}${markDone}${ADE.gbtn(ADE.can(p, 'release-membership'), 'Add to release', 'addToReleaseOpen', { pid: p.id, wid: it.id }, 'quiet', 'flag')}${G('order').ok && sibs.length > 1 ? `${si > 0 ? ADE.btn('Move up', 'workMove', { pid: p.id, wid: it.id, dir: -1 }, 'quiet', 'arrow_upward') : ''}${si < sibs.length - 1 ? ADE.btn('Move down', 'workMove', { pid: p.id, wid: it.id, dir: 1 }, 'quiet', 'arrow_downward') : ''}` : ''}`;
    const margin = [
      ...(it.questions || []).map((qq) => {
    ...
    return `<article class="doc">
      <div class="doc__main">
        ...
        ${docSection('Purpose', para(it.purpose, 'Not written yet.'), true, it.purpose, E('purpose', 'purpose'))}
        ${docSection(`In scope (${it.inScope.length})`, bullets(it.inScope, 'Nothing listed yet.', anchor), true, it.inScope.join(' · '), E('inScope', 'in scope'))}
        ${docSection(`Out of scope (${it.outScope.length})`, bullets(it.outScope, 'Nothing excluded yet.'), !!it.outScope.length, it.outScope.join(' · '), E('outScope', 'out of scope'))}
        ${docSection(`Acceptance criteria (${it.criteria.length})`, it.criteria.length ? `<ol class="criteria">${it.criteria.map((c) => `<li><span class="meta">${esc(c.id)}</span> ${esc(c.text)}</li>`).join('')}</ol>` : para('', 'No acceptance criteria yet. Readiness needs at least one.'), true, it.criteria.map((c) => c.text).join(' · '), E('criteria', 'acceptance criteria'))}
        ${docSection('Done when', `<ul class="checks done-when">${doneList.map((d) => `<li class="${it.status === 'done' ? 'is-ok' : ''}">${icon(it.status === 'done' ? 'check_circle' : 'radio_button_unchecked')}<span>${esc(d.text)}<small>${esc(d.from)}</small></span></li>`).join('')}</ul><p class="meta">Computed from the acceptance criteria, the ${esc((ADE.profile(p, ADE.defaultProfile(p, it)) || {}).title || '')} profile's workflow and the human steps policy requires. Use the edit button to add conditions by hand.</p>`, false, `${doneList.length} condition${doneList.length === 1 ? '' : 's'}`, E('doneExtra', 'extra done-when conditions'))}
        ${ch.length ? docSection(`Children (${ch.length})`, `<ul class="tree tree--inline" role="tree" aria-label="Children">${ch.map((c) => `<li class="tree__item" role="treeitem" aria-level="1"><div class="tree__row"><span class="tree__leaf"></span><span class="tree__main">${ADE.a(c.title, ADE.plink(p.id, `work/${c.id}`), 'tree__title')}</span><span class="meta tree__id">${esc(c.id)}</span>${ADE.person(c.owner, true)}${ADE.workState(p, c)}<span class="tree__cost meta">${(() => { const cr = ADE.roll(p, c.id); return cr.lo || cr.hi ? range([cr.lo, cr.hi]) : 'No estimate'; })()}</span></div></li>`).join('')}</ul>${it.status === 'done' && !cc.ok ? '' : ''}<p class="meta">A parent can be marked done only when every child is done or deferred. Adding a child to a done item reopens it.</p>`, true, `${r.done} of ${r.leaves} leaf items done`) : ''}
        ...
        ${docSection(`Attachments (${it.attachments.length})`, `<ul class="plain attach-list">${it.attachments.map((a) => { const s = ADE.source(p, a.src); if (!s) return ''; const newer = ADE.newerRevision(p, a); return `<li><button class="link" type="button" data-act="sourceOpen" data-pid="${esc(p.id)}" data-src="${esc(s.id)}">${esc(s.title)}</button> <span class="meta">pinned @ ${esc(a.rev)}</span> ${ADE.badge(s.status)}${newer ? ` ${ADE.tone(`Newer revision ${newer} available`, 'info', 'history')} ${gAttach.ok ? ADE.btn(`Use ${newer}`, 'attachRefresh', { pid: p.id, wid: it.id, src: s.id }, 'quiet', 'sync') : ''}` : ''} ${gAttach.ok ? ADE.btn('Remove', 'detachOpen', { pid: p.id, wid: it.id, src: s.id }, 'quiet', 'remove') : ''}</li>`; }).join('') || '<li class="meta">Nothing attached. Attach a PRD, designs, architecture notes or requirements; they become project sources.</li>'}</ul>
          ...
          <div class="row-actions">${ADE.gbtn(gAttach, 'Attach', 'attachOpen', { pid: p.id, wid: it.id }, 'outline', 'add')}</div>`, false, it.attachments.map((a) => (ADE.source(p, a.src) || {}).title).filter(Boolean).join(', ') || 'None')}
        ${docSection('Forecast and actuals', `<div class="card cost cost--inline">${ADE.costGauge(r, Math.max(r.hi, 1) * 1.1)}<p class="meta">${ch.length ? 'Rolled up from children; each child counted once.' : it.forecast ? `Forecast ${range(it.forecast)} · spent ${eur(r.actual)}` : 'No estimate yet — shown as unknown, not zero.'}</p>${ADE.spendKv(ADE.spend(p, it.id))}${it.evidence ? `<p><strong>Evidence recorded:</strong> ${esc(it.evidence)}</p>` : ''}</div>`, false, ch.length ? `${range([r.lo, r.hi])}${r.unknown ? ` + ${r.unknown} unestimated` : ''}` : it.forecast ? `${range(it.forecast)} · spent ${eur(it.actual)}` : 'No estimate', ch.length ? '' : E('forecast', 'forecast'))}
        ${(it.risks || []).length ? docSection(`Risks (${it.risks.length})`, `<div class="grid grid--2">${it.risks.map((id) => ADE.riskById(p, id)).filter(Boolean).map((x) => ADE.riskCard(p, x)).join('')}</div>`, false, it.risks.map((id) => ADE.riskById(p, id)?.title).filter(Boolean).join(' · ')) : ''}
        ${ADE.runsOf(p, it.id).length ? docSection('Agent runs', ADE.runsOf(p, it.id).map((x) => ADE.runRow(p, x)).join(''), !!run, `${ADE.runsOf(p, it.id).length} runs`) : ''}
        ${docSection(`Comments (${(it.comments || []).length})`, ADE.comments(p, 'work', it.id, it.comments || []), false, (it.comments || []).length ? `${ADE.personName(it.comments[it.comments.length - 1].by)}: ${it.comments[it.comments.length - 1].text}` : 'Leave a note for people or @agent')}
      </div>
  ...
  scr.knowledge = function (p, q) {
    ...
    return `${head}${chain}${nav}
      <form class="qa" data-form data-submit="knowledgeAsk" data-pid="${esc(p.id)}">
        ...
        <div class="qa__row"><input id="qa-input" class="input" name="q" value="${esc(q.q || '')}" placeholder="e.g. ${esc(p.questions[0] ? p.questions[0].q : 'What is out of scope?')}">${ADE.btn('Ask', 'knowledgeAskBtn', { pid: p.id }, 'primary', 'search')}</div>
        <p class="meta">${icon('lock')}Read-only. Answers cite current project sources and decisions, and never change the project.</p>
        <div class="chips">${p.questions.map((k) => chipLink(k.q, ADE.plink(p.id, 'knowledge', { q: k.q }), q.q === k.q)).join('')}</div>
      </form>
  ...
  scr.config = function (p, q) {
    ...
    return `${ADE.pageHeader({ eyebrow: `${p.name} · configuration`, title: meta.title, sub: meta.sub })}
      ...
      <section class="section"><div class="section__head"><h2>What revision ${c.revision} says</h2><span class="row-actions">${ADE.btn('Active JSON', 'contractJson', { pid: p.id, key }, 'quiet', 'code')}${ADE.btn('README', 'contractReadme', { pid: p.id, key }, 'quiet', 'menu_book')}${ADE.btn('History', 'contractHistory', { pid: p.id, key }, 'quiet', 'history')}</span></div>
        <ul class="sentences">${summary.map((s) => `<li>${esc(s.text)} <span class="meta">${esc(s.rule)}</span></li>`).join('')}</ul><p class="meta">Plain-language summary generated from the active contract.${ADE.gen(`contract:${p.id}:${key}`)}</p></section>
      ...
      ${key === 'policy' ? section('Who may do what', `<table class="table"><thead><tr><th>Role</th><th>May do</th></tr></thead><tbody>${ADE.grantTable().map(([r, t]) => `<tr><td><strong>${esc(r)}</strong></td><td>${esc(t)}</td></tr>`).join('')}</tbody></table><p class="meta">Grants are separate from policy verdicts: a member may start an agent, and policy then allows it, routes it for a decision or denies it. In this prototype the grant table is fixed; unauthorised actions stay visible, disabled, with the reason.</p>`) : ''}`;
  };
