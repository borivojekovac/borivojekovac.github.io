...
(function (root) {
  ...
  const borrowbox = {
    ...
    work: [
      ...
      W('W-132', 'W-130', 'Member loan history page', { readiness: 'ready', forecast: [200, 380], owner: 'sofia', customerVisible: true, purpose: 'Members see what they borrowed and when they returned it.', inScope: ['A list of the member\'s past and current loans with due and return dates'], outScope: ['Reading recommendations', 'Exporting the history'], ac: ['A signed-in member sees every loan of the last 12 months.', 'Current loans show their due date first.'] }),
      ...
      W('W-220', 'W-200', 'Reminder preferences', { readiness: 'ready', forecast: [70, 130], owner: 'tomas', customerVisible: true, purpose: 'Members choose whether and when they get due-date reminders.', inScope: ['An on/off switch and a choice of two days or one day before'], outScope: ['The delivery channel (decided in W-210)'], ac: ['A member can turn reminders off.', 'The chosen timing is saved on the member account.'] }),
      ...
      W('W-321', 'W-320', 'Copies per title', { readiness: 'ready', forecast: [80, 140], owner: 'elena', purpose: 'Staff see how many copies of each title the library owns and where they are.', inScope: ['A staff table of titles with owned, lent and on-shelf counts'], outScope: ['Buying decisions', 'Member-facing counts'], ac: ['Each title shows owned, lent and on-shelf copies.', 'Counts match the loans recorded that day.'] }),
  ...
  const skillhaven = {
    ...
    work: [
      ...
      S('W-232', 'W-230', 'Sessions without a trainer', { readiness: 'ready', forecast: [70, 110], owner: 'elena', customerVisible: true, purpose: 'Coordinators find sessions that still need a trainer before learners book them.', inScope: ['A filter on the sessions list for sessions with no trainer assigned'], outScope: ['Assigning trainers automatically'], ac: ['Sessions without a trainer appear under the filter.', 'Assigning a trainer removes the session from it.'] }),
      ...
      S('W-312', 'W-310', 'Export roster to PDF', { readiness: 'ready', forecast: [60, 100], owner: 'sofia', customerVisible: true, purpose: 'Trainers print a roster for rooms without a screen.', inScope: ['A PDF export of the session roster'], outScope: ['Attendance marking (W-313)'], ac: ['The PDF lists every booked learner.'] }),
