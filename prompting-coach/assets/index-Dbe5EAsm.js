...
`);u=w.pop()||"";for(const v of w){const x=v.trim();if(!(!x||x==="data: [DONE]")&&x.startsWith("data: "))try{const C=JSON.parse(x.slice(6)),T=(f=(h=C.choices)==null?void 0:h[0])==null?void 0:f.delta;T!=null&&T.content&&(yield{content:T.content,done:!1,usage:null}),C.usage&&(m={prompt:C.usage.prompt_tokens,completion:C.usage.completion_tokens,total:C.usage.total_tokens})}catch(C){console.warn("X/Grok: Failed to parse chunk",C)}}}}catch(p){throw p instanceof b?p:b.streamInterrupted(p.message)}finally{l.releaseLock()}}}S(le,"id","x"),S(le,"displayName","X (Grok)");const z={OPENAI:"openai",ANTHROPIC:"anthropic",GOOGLE:"google",X:"x"},ge={[z.OPENAI]:re,[z.ANTHROPIC]:ae,[z.GOOGLE]:ce,[z.X]:le};function kt(){return Object.values(ge).map(n=>({id:n.id,name:n.displayName,models:n.getModels(),defaultModel:n.getDefaultModel()}))}function Ue(n,e,t=null){const s=n==null?void 0:n.toLowerCase(),i=ge[s];if(!i)throw new b(`Unknown provider type: ${n}`,E.UNKNOWN,!1);const o=t||i.getDefaultModel();return new i(e,o)}function Le(n){const e=ge[n==null?void 0:n.toLowerCase()];return e?e.getModels():[]}const Q=kt();class St extends Ve{constructor(){super({title:"Settings"}),this.settings=L(),this.apiKeyInputs={}}createContent(){const e=document.createElement("div");return e.style.cssText="display: flex; flex-direction: column; gap: var(--pc-space-6);",e.appendChild(this.createThemeSection()),e.appendChild(this.createCoachLLMSection()),e.appendChild(this.createTestLLMSection()),e.appendChild(this.createApiKeysSection()),e}createThemeSection(){const e=document.createElement("div"),t=document.createElement("h3");t.className="text-title",t.style.cssText="margin-bottom: var(--pc-space-3);",t.textContent="Theme",e.appendChild(t);const s=document.createElement("div");return s.style.cssText="display: flex; gap: var(--pc-space-3);",["system","light","dark"].forEach(i=>{const o=document.createElement("label");o.style.cssText=`
    ...
    `;const t=document.createElement("div");t.style.cssText=`
      ...
      border-radius: 2px;
    `,e.appendChild(t);let s=!1,i=0,o=0;const r=l=>{s=!0,i=l.clientY,o=this.ratio,document.body.style.cursor="ns-resize",document.body.style.userSelect="none",l.preventDefault()},a=l=>{if(!s)return;const u=(l.clientY-i)/window.innerHeight;this.setRatio(o+u)},d=()=>{s&&(s=!1,document.body.style.cursor="",document.body.style.userSelect="")};return e.addEventListener("mousedown",r),document.addEventListener("mousemove",a),document.addEventListener("mouseup",d),e.addEventListener("touchstart",l=>{s=!0,i=l.touches[0].clientY,o=this.ratio,l.preventDefault()}),document.addEventListener("touchmove",l=>{if(!s)return;const u=(l.touches[0].clientY-i)/window.innerHeight;this.setRatio(o+u)}),document.addEventListener("touchend",()=>{s=!1}),e}createHeader(){const e=document.createElement("div");e.className="feedback-header",e.style.cssText=`
      display: flex;
      align-items: center;
    ...
    `,o.textContent=e.overall,i.appendChild(o),s.appendChild(i);const r=document.createElement("div");r.style.cssText="flex: 1;";const a=document.createElement("div");a.className="text-label",a.style.cssText="color: var(--pc-on-surface-variant); margin-bottom: 2px;",a.textContent=t?"🎉 Coaching Complete!":"Overall Assessment",r.appendChild(a);const d=document.createElement("div");if(d.className="text-body",d.textContent=e.description,r.appendChild(d),s.appendChild(r),this.contentEl.appendChild(s),e.targetPrinciple&&!t){const c=document.createElement("div");c.className="target-principle-badge",c.style.cssText=`
      ...
      `,this.contentEl.appendChild(c)}const l=document.createElement("div");if(l.className="feedback-text",l.style.cssText=`
      line-height: 1.6;
      color: var(--pc-on-surface);
    `,l.textContent=e.feedback,this.contentEl.appendChild(l),e.scores&&e.scores.length>0){const c=document.createElement("details");c.style.cssText="margin-top: var(--pc-space-4);";const u=document.createElement("summary");u.style.cssText=`
        cursor: pointer;
        color: var(--pc-primary);
        font-weight: 500;
        padding: var(--pc-space-2) 0;
      `,u.textContent="View all principle scores",c.appendChild(u);const m=document.createElement("div");m.style.cssText=`
        display: flex;
        flex-direction: column;
        gap: var(--pc-space-2);
...
`),truncated:i,count:e.length}}class Ut{constructor(){this.element=null,this.textarea=null,this.preview=null,this.isPreviewMode=!1,this.promptText="",this.initialized=!1,this.isTestRunning=!1,this.isCoachRunning=!1,this.onSwitchToResults=null,this.resultsTab=null,this.feedbackPanel=null,this.onShowFeedback=null,this.keyboardHandler=null,this.promptHistory=new Dt(50)}render(){if(this.element)return this.element;this.element=document.createElement("div"),this.element.className="prompt-tab",this.element.setAttribute("role","tabpanel"),this.element.setAttribute("id","prompt-panel"),this.element.style.cssText=`
    ...
    `,this.testBtn.addEventListener("click",()=>this.runTest()),e.appendChild(this.testBtn),this.coachBtn=document.createElement("button"),this.coachBtn.className="btn btn-text",this.coachBtn.title="Get coaching feedback (Ctrl+Shift+Enter)",this.coachBtn.innerHTML=`
      ...
      Help
    `,this.coachBtn.addEventListener("click",()=>this.runCoach()),e.appendChild(this.coachBtn),e}setMode(e){this.isPreviewMode!==e&&(this.isPreviewMode=e,this.isPreviewMode?(this.textarea.style.display="none",this.preview.style.display="block",this.preview.innerHTML=pe(this.promptText)||'<p style="color: var(--pc-on-surface-variant);">Nothing to preview</p>',this.applyMarkdownStyles(this.preview),this.editBtn.style.background="transparent",this.editBtn.style.color="var(--pc-on-surface)",this.previewBtn.style.background="var(--pc-primary)",this.previewBtn.style.color="var(--pc-on-primary)"):(this.textarea.style.display="block",this.preview.style.display="none",this.textarea.focus(),this.editBtn.style.background="var(--pc-primary)",this.editBtn.style.color="var(--pc-on-primary)",this.previewBtn.style.background="transparent",this.previewBtn.style.color="var(--pc-on-surface)"))}async copyPrompt(){if(!this.promptText.trim()){g("Nothing to copy");return}try{await navigator.clipboard.writeText(this.promptText),g("Copied to clipboard")}catch{g("Failed to copy")}}async loadPrompt(){try{const e=await P();this.promptText=e.promptText||"",this.textarea&&(this.textarea.value=this.promptText),await this.promptHistory.load(),this.promptText&&this.promptHistory.push(this.promptText),this.initKeyboardHandler()}catch(e){console.error("Failed to load prompt:",e)}}initKeyboardHandler(){this.keyboardHandler||(this.keyboardHandler=new Rt,this.keyboardHandler.init(e=>this.handleKeyboardAction(e),()=>this.isKeyboardActive()))}isKeyboardActive(){return this.element&&this.element.offsetParent!==null&&!this.isPreviewMode}handleKeyboardAction(e){switch(e){case"test":this.runTest();break;case"coach":this.runCoach();break;case"undo":this.handleUndo();break;case"redo":this.handleRedo();break;default:console.warn("Unknown keyboard action:",e)}}handleUndo(){const e=this.promptHistory.undo();e!==null&&(this.promptText=e,this.textarea&&(this.textarea.value=e),Z(e),g("Undo"))}handleRedo(){const e=this.promptHistory.redo();e!==null&&(this.promptText=e,this.textarea&&(this.textarea.value=e),Z(e),g("Redo"))}async runTest(){var m,h,f;if(this.isTestRunning){g("Test already in progress");return}const e=this.promptText.trim();if(!e){g("Please enter a prompt first");return}if(!ie()){g("Please configure an API key in Settings");return}const t=L(),s=t.testProvider||"openai",i=t.apiKeys[s],o=t.testModel||null;if(!i){g(`No API key configured for ${s}`);return}let r;try{r=Ue(s,i,o)}catch(p){g(p.message||"Failed to create provider");return}let a=e,d=null;try{const{context:p,truncated:y,count:w}=await De();p&&(a=`${p}

---

${e}`,d={count:w,truncated:y},y&&g("Attachments truncated due to size limit","warning"))}catch(p){console.warn("Failed to get attachment context:",p)}this.isTestRunning=!0,this.testBtn.disabled=!0,this.setButtonLoading(this.testBtn,!0);const l=pt({promptSnapshot:e,provider:r.getProviderName(),model:r.getModelName(),status:"streaming",attachmentCount:(d==null?void 0:d.count)||0});this.onSwitchToResults&&this.onSwitchToResults();const c=(m=this.resultsTab)==null?void 0:m.addResult(l);await lt(l);const u=performance.now();await $e(r.getProviderName(),r.getModelName(),a.length);try{for await(const p of r.streamCompletion(a))if(p.content&&(c==null||c.appendChunk(p.content)),p.done&&p.usage){const y=Math.round(performance.now()-u);c==null||c.complete(p.usage,y),await Ee(l.id,{responseText:(c==null?void 0:c.getResult().responseText)||"",tokens:p.usage,durationMs:y,status:"complete"}),await ze(r.getProviderName(),r.getModelName(),((h=c==null?void 0:c.getResult().responseText)==null?void 0:h.length)||0,p.usage,y),(f=this.resultsTab)==null||f.updateSessionStats()}}catch(p){const y=Math.round(performance.now()-u),w=p instanceof b?p.userMessage:p.message||"Unknown error";c==null||c.setError(w,y),await Ee(l.id,{responseText:(c==null?void 0:c.getResult().responseText)||"",error:w,durationMs:y,status:"error"}),await se(r.getProviderName(),r.getModelName(),w,y),g(w)}finally{this.isTestRunning=!1,this.testBtn.disabled=!1,this.setButtonLoading(this.testBtn,!1)}}setOnSwitchToResults(e){this.onSwitchToResults=e}setResultsTab(e){this.resultsTab=e}setOnShowFeedback(e){this.onShowFeedback=e}async runCoach(){if(this.isCoachRunning){g("Coach evaluation in progress");return}const e=this.promptText.trim();if(!e){g("Please enter a prompt first");return}if(!ie()){g("Please configure an API key in Settings");return}const t=L(),s=t.coachProvider||"openai",i=t.apiKeys[s],o=t.coachModel||null;if(!i){g(`No API key configured for ${s}`);return}let r="",a=!1;try{const{context:l,truncated:c}=await De();r=l,a=c,c&&g("Attachments truncated due to size limit","warning")}catch(l){console.warn("Failed to get attachment context:",l)}this.isCoachRunning=!0,this.coachBtn.disabled=!0,this.setButtonLoading(this.coachBtn,!0);const d=performance.now();try{const l=new Bt(s,i,o),c=de.get(),u=await l.evaluateWithRetry(e,c,{attachmentContext:r,attachmentsTruncated:a}),m=Math.round(performance.now()-d),h=ht({promptSnapshot:e,scores:u.scores,overall:u.overall,description:u.description,feedback:u.feedback,provider:l.getProviderName(),model:l.getModelName(),durationMs:m,targetPrinciple:u.targetPrinciple,complete:u.complete});await ut(h),this.onShowFeedback&&this.onShowFeedback(h),console.log("Coach: Evaluation complete",h)}catch(l){const c=l.message||"Coach evaluation failed";g(c),console.error("Coach: Error",l),await se(t.coachProvider||"openai",t.coachModel||"unknown",c,Math.round(performance.now()-d))}finally{this.isCoachRunning=!1,this.coachBtn.disabled=!1,this.setButtonLoading(this.coachBtn,!1)}}setButtonLoading(e,t){if(!document.getElementById("btn-spinner-styles")){const o=document.createElement("style");o.id="btn-spinner-styles",o.textContent=`
    ...
    `,l.textContent=this.entry.feedback,this.element.appendChild(l),this.entry.feedback.length>200){const c=document.createElement("button");c.style.cssText=`
      ...
      `,c.textContent=this.isExpanded?"Show less":"Show more",c.addEventListener("click",()=>{this.isExpanded=!this.isExpanded,l.style.cssText=`
          ...
          font-size: 0.9rem;
          ${this.isExpanded?"":"display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden;"}
        `,c.textContent=this.isExpanded?"Show less":"Show more"}),this.element.appendChild(c)}if(this.entry.scores&&this.entry.scores.length>0){const c=document.createElement("details");c.style.cssText="margin-top: var(--pc-space-2);";const u=document.createElement("summary");u.style.cssText=`
        cursor: pointer;
        font-size: 0.8rem;
        color: var(--pc-on-surface-variant);
        display: flex;
      ...
      `,this.entry.scores.forEach(h=>{const f=document.createElement("span");f.style.cssText=`
          ...
          border-radius: var(--pc-radius-full);
          font-size: 0.75rem;
        `,f.innerHTML=`<span style="text-transform: capitalize;">${h.principle}</span>: <strong>${h.score}</strong>`,m.appendChild(f)}),c.appendChild(m),this.element.appendChild(c)}if(this.entry.promptSnapshot){const c=document.createElement("details");c.style.cssText="margin-top: var(--pc-space-2);";const u=document.createElement("summary");u.style.cssText=`
        cursor: pointer;
        font-size: 0.8rem;
        color: var(--pc-on-surface-variant);
        display: flex;
