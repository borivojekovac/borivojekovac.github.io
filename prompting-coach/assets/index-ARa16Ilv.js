var tt=Object.defineProperty;var st=(n,e,t)=>e in n?tt(n,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):n[e]=t;var S=(n,e,t)=>st(n,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))s(i);new MutationObserver(i=>{for(const o of i)if(o.type==="childList")for(const r of o.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&s(r)}).observe(document,{childList:!0,subtree:!0});function t(i){const o={};return i.integrity&&(o.integrity=i.integrity),i.referrerPolicy&&(o.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?o.credentials="include":i.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function s(i){if(i.ep)return;i.ep=!0;const o=t(i);fetch(i.href,o)}})();const J="logs";let q=null;function nt(n){q=n}function G(n){return{id:crypto.randomUUID(),timestamp:new Date().toISOString(),type:n.type||"request",provider:n.provider||"",model:n.model||"",promptLength:n.promptLength||0,responseLength:n.responseLength||null,tokens:n.tokens||null,error:n.error||null,durationMs:n.durationMs||0}}async function Y(n){if(!q){console.warn("Logger: Database not initialized");return}if(!q.objectStoreNames.contains(J)){console.warn("Logger: Logs store not found - DB may need upgrade. Clear site data to fix.");return}return new Promise((e,t)=>{try{const s=q.transaction(J,"readwrite");s.objectStore(J).add(n),s.oncomplete=()=>e(),s.onerror=()=>t(s.error)}catch(s){console.error("Logger: Failed to save log",s),e()}})}async function Ke(n,e,t){const s=G({type:"request",provider:n,model:e,promptLength:t});return await Y(s),s}async function je(n,e,t,s,i){const o=G({type:"response",provider:n,model:e,responseLength:t,tokens:s,durationMs:i});await Y(o)}async function ne(n,e,t,s){const i=G({type:"error",provider:n,model:e,error:t,durationMs:s});await Y(i)}async function it(n,e="",t=""){const s=G({type:"skip",provider:e,model:t,error:null,durationMs:0});s.principleId=n,await Y(s)}const ie="promptcoach",ot=5;let P=null;const Ee=["sessions","logs","feedback","promptHistory","attachments"];async function T(){if(P){const n=Ee.filter(e=>!P.objectStoreNames.contains(e));if(n.length===0)return P;console.warn("IndexedDB missing stores:",n,"- resetting database"),P.close(),P=null,await rt()}return new Promise((n,e)=>{const t=indexedDB.open(ie,ot);t.onerror=()=>e(t.error),t.onsuccess=()=>{P=t.result;const s=Ee.filter(i=>!P.objectStoreNames.contains(i));if(s.length>0){console.warn("IndexedDB still missing stores after open:",s,"- deleting and retrying"),P.close(),P=null,indexedDB.deleteDatabase(ie),setTimeout(()=>{T().then(n).catch(e)},100);return}nt(P),n(P)},t.onupgradeneeded=s=>{const i=s.target.result;if(!i.objectStoreNames.contains("sessions")){const o=i.createObjectStore("sessions",{keyPath:"id"});o.createIndex("createdAt","createdAt",{unique:!1}),o.createIndex("updatedAt","updatedAt",{unique:!1})}if(!i.objectStoreNames.contains("logs")){const o=i.createObjectStore("logs",{keyPath:"id"});o.createIndex("timestamp","timestamp",{unique:!1}),o.createIndex("type","type",{unique:!1}),o.createIndex("provider","provider",{unique:!1})}if(!i.objectStoreNames.contains("feedback")){const o=i.createObjectStore("feedback",{keyPath:"id"});o.createIndex("sessionId","sessionId",{unique:!1}),o.createIndex("timestamp","timestamp",{unique:!1})}if(i.objectStoreNames.contains("promptHistory")||i.createObjectStore("promptHistory",{keyPath:"id"}),!i.objectStoreNames.contains("attachments")){const o=i.createObjectStore("attachments",{keyPath:"id"});o.createIndex("addedAt","addedAt",{unique:!1}),o.createIndex("filename","filename",{unique:!1})}}})}function rt(){return new Promise(n=>{const e=indexedDB.deleteDatabase(ie);e.onsuccess=()=>n(),e.onerror=()=>n(),e.onblocked=()=>{console.warn("IndexedDB delete blocked - close other tabs"),n()}})}async function fe(n,e){const t=await T();return new Promise((s,i)=>{const a=t.transaction(n,"readonly").objectStore(n).get(e);a.onerror=()=>i(a.error),a.onsuccess=()=>s(a.result)})}async function z(n,e){const t=await T();return new Promise((s,i)=>{const a=t.transaction(n,"readwrite").objectStore(n).put(e);a.onerror=()=>i(a.error),a.onsuccess=()=>s()})}async function ge(n){const e=await T();return new Promise((t,s)=>{const r=e.transaction(n,"readonly").objectStore(n).getAll();r.onerror=()=>s(r.error),r.onsuccess=()=>t(r.result)})}async function at(n,e){const t=await T();return new Promise((s,i)=>{const a=t.transaction(n,"readwrite").objectStore(n).delete(e);a.onerror=()=>i(a.error),a.onsuccess=()=>s()})}async function lt(n){const e=await T();return new Promise((t,s)=>{const r=e.transaction(n,"readwrite").objectStore(n).clear();r.onerror=()=>s(r.error),r.onsuccess=()=>t()})}const qe="promptcoach_appstate",V="promptcoach_settings",Ve="promptcoach_current_session",Ce={currentTab:"prompt",firstRunCompleted:!1,feedbackPanelPinned:!1,feedbackPanelRatio:.5},F={theme:"system",apiKeys:{openai:null,anthropic:null,google:null,x:null},coachProvider:"openai",coachModel:null,testProvider:"openai",testModel:null};function W(){try{const n=localStorage.getItem(qe);if(n)return{...Ce,...JSON.parse(n)}}catch(n){console.error("Failed to load AppState:",n)}return{...Ce}}function ct(n){try{["prompt","feedback","attachments","results"].includes(n.currentTab)||(n.currentTab="prompt"),(n.feedbackPanelRatio<.2||n.feedbackPanelRatio>.8)&&(n.feedbackPanelRatio=.5),localStorage.setItem(qe,JSON.stringify(n))}catch(e){console.error("Failed to save AppState:",e)}}function I(n,e){const t=W();return t[n]=e,ct(t),t}function N(){var n;try{const e=localStorage.getItem(V);if(e){const t=JSON.parse(e);return t.apiKey&&!((n=t.apiKeys)!=null&&n.openai)&&(t.apiKeys=t.apiKeys||{},t.apiKeys.openai=t.apiKey,delete t.apiKey,localStorage.setItem(V,JSON.stringify(t)),console.log("Settings: Migrated legacy apiKey to apiKeys.openai")),t.model&&!t.testModel&&(t.testModel=t.model,t.coachModel=t.model,delete t.model,localStorage.setItem(V,JSON.stringify(t)),console.log("Settings: Migrated legacy model to testModel/coachModel")),{...F,...t,apiKeys:{...F.apiKeys,...t.apiKeys}}}}catch(e){console.error("Failed to load Settings:",e)}return{...F,apiKeys:{...F.apiKeys}}}function Ue(n){try{["system","light","dark"].includes(n.theme)||(n.theme="system");const t=["openai","anthropic","google","x"];t.includes(n.coachProvider)||(n.coachProvider="openai"),t.includes(n.testProvider)||(n.testProvider="openai"),localStorage.setItem(V,JSON.stringify(n))}catch(e){console.error("Failed to save Settings:",e)}}function Ge(n,e){const t=N();return n==="apiKeys"?t.apiKeys={...t.apiKeys,...e}:t[n]=e,Ue(t),t}function oe(){const n=N();return Object.values(n.apiKeys).some(e=>e&&e.trim()!=="")}function ve(){try{const n=localStorage.getItem(Ve);if(n)return JSON.parse(n).sessionId}catch(n){console.error("Failed to load current session ID:",n)}return null}function dt(n){try{localStorage.setItem(Ve,JSON.stringify({sessionId:n}))}catch(e){console.error("Failed to save current session ID:",e)}}async function pt(){const n={id:crypto.randomUUID(),createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),promptText:"",promptHistory:[],results:[],totalTokens:0,lastScore:null,lastDescription:null,feedbackCount:0};return await z("sessions",n),dt(n.id),n}async function M(){await T();const n=ve();if(n){const e=await fe("sessions",n);if(e)return e}return pt()}async function $(n){const t={...await M(),...n,updatedAt:new Date().toISOString()};return await z("sessions",t),t}let Z=null;function Q(n){Z&&clearTimeout(Z),Z=setTimeout(async()=>{await $({promptText:n})},500)}async function ht(n){var t;const e=await M();return e.results||(e.results=[]),e.results.push(n),(t=n.tokens)!=null&&t.total&&(e.totalTokens=(e.totalTokens||0)+n.tokens.total),$({results:e.results,totalTokens:e.totalTokens})}async function ke(){return(await M()).results||[]}async function ut(){return $({results:[],totalTokens:0})}async function Te(n,e){const t=await M();if(!t.results)return t;const s=t.results.findIndex(i=>i.id===n);return s!==-1?(t.results[s]={...t.results[s],...e},e.tokens&&(t.totalTokens=t.results.reduce((i,o)=>{var r;return i+(((r=o.tokens)==null?void 0:r.total)||0)},0)),$({results:t.results,totalTokens:t.totalTokens})):t}function mt(n={}){return{id:crypto.randomUUID(),timestamp:new Date().toISOString(),promptSnapshot:n.promptSnapshot||"",provider:n.provider||"",model:n.model||"",responseText:n.responseText||"",tokens:n.tokens||{prompt:0,completion:0,total:0},durationMs:n.durationMs||0,error:n.error||null,status:n.status||"streaming"}}function ft(n={}){return{id:crypto.randomUUID(),sessionId:ve(),timestamp:new Date().toISOString(),promptSnapshot:n.promptSnapshot||"",scores:n.scores||[],overall:n.overall||0,description:n.description||"",feedback:n.feedback||"",provider:n.provider||"",model:n.model||"",durationMs:n.durationMs||0,targetPrinciple:n.targetPrinciple||null,complete:n.complete||!1}}async function gt(n){await T();try{await z("feedback",n)}catch(t){console.warn("Failed to save feedback to IndexedDB - DB may need upgrade. Clear site data to fix.",t)}const e=await M();return await $({lastScore:n.overall,lastDescription:n.description,feedbackCount:(e.feedbackCount||0)+1}),n}async function vt(){await T();const n=ve();if(!n)return[];try{return(await ge("feedback")).filter(t=>t.sessionId===n).sort((t,s)=>new Date(s.timestamp)-new Date(t.timestamp))}catch(e){return console.warn("Failed to get feedback from IndexedDB - DB may need upgrade.",e),[]}}const Se="data-theme";function re(n){const e=document.documentElement;n==="system"?e.removeAttribute(Se):e.setAttribute(Se,n);const t=document.querySelector('meta[name="theme-color"]');if(t){const s=n==="dark"||n==="system"&&window.matchMedia("(prefers-color-scheme: dark)").matches;t.setAttribute("content",s?"#1C1B1F":"#6750A4")}}function yt(n){Ge("theme",n),re(n)}function bt(){const n=N();re(n.theme),window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",()=>{N().theme==="system"&&re("system")})}class xt{constructor(){this.element=null,this.ringEl=null,this.scoreEl=null,this.score=null,this.description=null,this.onClick=null}render(){this.element=document.createElement("div"),this.element.className="score-badge",this.element.style.cssText=`
      display: flex;
      align-items: center;
    ...
    `,this.scoreEl.textContent="—",e.appendChild(this.scoreEl),this.element.appendChild(e),this.element}setScore(e,t){this.score=e,this.description=t,this.scoreEl.textContent=Math.round(e);const i=100.53*(1-e/100);this.ringEl.setAttribute("stroke-dashoffset",i.toString()),e>=80?this.ringEl.setAttribute("stroke","var(--pc-primary)"):e>=60?this.ringEl.setAttribute("stroke","#F59E0B"):this.ringEl.setAttribute("stroke","#EF4444"),this.element.title=t||`Score: ${Math.round(e)}`}clear(){this.score=null,this.description=null,this.scoreEl.textContent="—",this.ringEl.setAttribute("stroke-dashoffset","100.53"),this.ringEl.setAttribute("stroke","var(--pc-primary)"),this.element.title="Not evaluated"}setOnClick(e){this.onClick=e}getScore(){return this.score}}class wt{constructor(e={}){this.onMenuClick=e.onMenuClick||null,this.onScoreClick=e.onScoreClick||null,this.element=null,this.scoreBadge=null}render(){this.element=document.createElement("header"),this.element.className="ribbon",this.element.style.cssText=`
      display: flex;
      align-items: center;
      height: 56px;
    ...
    `,t.textContent="Prompt Coach",this.element.appendChild(t),this.scoreBadge=new xt,this.scoreBadge.setOnClick(()=>{this.onScoreClick&&this.onScoreClick()});const s=document.createElement("div");return s.style.cssText=`
      display: flex;
      align-items: center;
      justify-content: center;
      width: 28px;
    ...
    `,Et.forEach(e=>{const t=this.createTabButton(e);this.tabButtons[e.id]=t,this.element.appendChild(t)}),this.updateActiveTab(),this.element}createTabButton(e){const t=document.createElement("button");t.className="tab-button",t.setAttribute("role","tab"),t.setAttribute("aria-selected",e.id===this.currentTab),t.setAttribute("aria-controls",`${e.id}-panel`),t.style.cssText=`
      ...
      min-height: 44px;
      -webkit-tap-highlight-color: transparent;
    `;const s=document.createElement("span");s.className="tab-icon",s.innerHTML=this.getIcon(e.icon);const i=document.createElement("span");return i.className="tab-label",i.style.cssText="font-size: 0.75rem; font-weight: 500;",i.textContent=e.label,t.appendChild(s),t.appendChild(i),t.addEventListener("click",()=>this.selectTab(e.id)),t}selectTab(e){this.currentTab!==e&&(this.currentTab=e,this.updateActiveTab(),this.onTabChange&&this.onTabChange(e))}updateActiveTab(){Object.entries(this.tabButtons).forEach(([e,t])=>{const s=e===this.currentTab;t.setAttribute("aria-selected",s),t.style.color=s?"var(--pc-primary)":"var(--pc-on-surface-variant)"})}setActiveTab(e){this.currentTab=e,this.updateActiveTab()}}let A=null;const kt=3e3;function Tt(){return A||(A=document.createElement("div"),A.className="toast-container",A.setAttribute("role","status"),A.setAttribute("aria-live","polite"),document.body.appendChild(A)),A}function f(n,e=kt){const t=Tt(),s=document.createElement("div");s.className="toast",s.textContent=n,t.appendChild(s);const i=setTimeout(()=>{Pe(s)},e);return s.addEventListener("click",()=>{clearTimeout(i),Pe(s)}),s}function Pe(n){n.classList.add("toast-exit"),n.addEventListener("animationend",()=>{n.parentNode&&n.parentNode.removeChild(n)})}function Me(){f("Coming soon")}class Ye{constructor(e={}){this.title=e.title||"",this.onClose=e.onClose||null,this.element=null,this.overlay=null,this.handleKeyDown=this.handleKeyDown.bind(this)}render(){this.overlay=document.createElement("div"),this.overlay.className="overlay",this.overlay.addEventListener("click",s=>{s.target===this.overlay&&this.close()}),this.element=document.createElement("div"),this.element.className="dialog",this.element.setAttribute("role","dialog"),this.element.setAttribute("aria-modal","true"),this.element.setAttribute("aria-labelledby","dialog-title");const e=document.createElement("div");e.className="dialog-header";const t=document.createElement("h2");return t.id="dialog-title",t.className="dialog-title",t.textContent=this.title,e.appendChild(t),this.content=document.createElement("div"),this.content.className="dialog-content",this.actions=document.createElement("div"),this.actions.className="dialog-actions",this.element.appendChild(e),this.element.appendChild(this.content),this.element.appendChild(this.actions),this.overlay.appendChild(this.element),this.overlay}setContent(e){typeof e=="string"?this.content.innerHTML=e:(this.content.innerHTML="",this.content.appendChild(e))}addAction(e,t,s="text"){const i=document.createElement("button");return i.className=`btn btn-${s}`,i.textContent=e,i.addEventListener("click",t),this.actions.appendChild(i),i}handleKeyDown(e){e.key==="Escape"&&this.close()}show(){this.overlay||this.render(),document.body.appendChild(this.overlay),document.addEventListener("keydown",this.handleKeyDown);const e=this.element.querySelector("button, input, textarea, select");e&&e.focus()}close(){document.removeEventListener("keydown",this.handleKeyDown),this.overlay&&this.overlay.parentNode&&this.overlay.parentNode.removeChild(this.overlay),this.onClose&&this.onClose()}}const _=class _{static getModels(){return[]}static getDefaultModel(){const e=this.getModels();return e.length>0?e[0].id:""}constructor(e,t){if(new.target===_)throw new Error("LLMProvider is abstract and cannot be instantiated directly");if(!e)throw new Error("API key is required");if(!t)throw new Error("Model is required");this.apiKey=e,this.model=t}getProviderName(){return this.constructor.displayName}getProviderId(){return this.constructor.id}getModelName(){return this.model}async*streamCompletion(e,t={}){throw new Error("Must implement streamCompletion()")}};S(_,"id","base"),S(_,"displayName","Base Provider");let R=_;const E={INVALID_API_KEY:"invalid_api_key",RATE_LIMITED:"rate_limited",NETWORK_ERROR:"network_error",SERVER_ERROR:"server_error",TIMEOUT:"timeout",STREAM_INTERRUPTED:"stream_interrupted",INVALID_RESPONSE:"invalid_response",UNKNOWN:"unknown",COACH_VALIDATION_FAILED:"coach_validation_failed",COACH_METHODOLOGY_MISSING:"coach_methodology_missing",COACH_EVALUATION_FAILED:"coach_evaluation_failed"},Ae={[E.INVALID_API_KEY]:"Invalid API key. Check Settings.",[E.RATE_LIMITED]:"Rate limit exceeded. Try again later.",[E.NETWORK_ERROR]:"Network error. Check connection.",[E.SERVER_ERROR]:"Server error. Try again later.",[E.TIMEOUT]:"Request timed out. Try again.",[E.STREAM_INTERRUPTED]:"Response interrupted.",[E.INVALID_RESPONSE]:"Invalid response from provider.",[E.UNKNOWN]:"An unexpected error occurred.",[E.COACH_VALIDATION_FAILED]:"Coach couldn't evaluate. Try again.",[E.COACH_METHODOLOGY_MISSING]:"Coaching methodology not configured.",[E.COACH_EVALUATION_FAILED]:"Coach couldn't evaluate. Try again."};class b extends Error{constructor(e,t=E.UNKNOWN,s=!1,i={}){super(e),this.name="LLMError",this.code=t,this.retryable=s,this.details=i,this.userMessage=Ae[t]||Ae[E.UNKNOWN]}static fromHttpStatus(e,t="",s=null){switch(e){case 401:return new b(`Authentication failed: ${t}`,E.INVALID_API_KEY,!1,{status:e,body:s});case 429:return new b(`Rate limit exceeded: ${t}`,E.RATE_LIMITED,!0,{status:e,body:s});case 500:case 502:case 503:case 504:return new b(`Server error: ${e} ${t}`,E.SERVER_ERROR,!0,{status:e,body:s});default:return new b(`HTTP error: ${e} ${t}`,E.UNKNOWN,!1,{status:e,body:s})}}static fromNetworkError(e){return e.name==="AbortError"?new b("Request was aborted",E.TIMEOUT,!0,{originalError:e.message}):new b(`Network error: ${e.message}`,E.NETWORK_ERROR,!0,{originalError:e.message})}static streamInterrupted(e="Unknown"){return new b(`Stream interrupted: ${e}`,E.STREAM_INTERRUPTED,!1,{reason:e})}}const Le="https://api.openai.com/v1/chat/completions";class ae extends R{static getModels(){return[{id:"gpt-4o",name:"GPT-4o",description:"Most capable, 128K context"},{id:"gpt-4o-mini",name:"GPT-4o Mini",description:"Fast and affordable"},{id:"gpt-4-turbo",name:"GPT-4 Turbo",description:"128K context"},{id:"gpt-4",name:"GPT-4",description:"8K context"},{id:"gpt-3.5-turbo",name:"GPT-3.5 Turbo",description:"Fast, 16K context"},{id:"o1",name:"o1",description:"Reasoning model"},{id:"o1-mini",name:"o1 Mini",description:"Fast reasoning"}]}constructor(e,t="gpt-4o-mini"){super(e,t)}async*streamCompletion(e,t={}){var h,g;const{maxTokens:s=2048,temperature:i=.7,systemPrompt:o=null}=t,r=[];o&&r.push({role:"system",content:o}),r.push({role:"user",content:e});const a={model:this.model,messages:r,max_tokens:s,temperature:i,stream:!0,stream_options:{include_usage:!0}};console.log("OpenAI: Sending request to",Le,"with model:",this.model);let d;try{d=await fetch(Le,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.apiKey}`},body:JSON.stringify(a)})}catch(p){throw console.error("OpenAI: Network error",p),b.fromNetworkError(p)}if(console.log("OpenAI: Response status",d.status),!d.ok){let p=null;try{p=await d.json(),console.error("OpenAI: Error response",p)}catch{}throw b.fromHttpStatus(d.status,d.statusText,p)}const c=d.body.getReader(),l=new TextDecoder;let u="",m=null;try{for(;;){const{done:p,value:y}=await c.read();if(p){yield{content:"",done:!0,usage:m};break}u+=l.decode(y,{stream:!0});const x=u.split(`
`);u=x.pop()||"";for(const v of x){const w=v.trim();if(!(!w||w==="data: [DONE]")&&w.startsWith("data: "))try{const C=JSON.parse(w.slice(6)),k=(g=(h=C.choices)==null?void 0:h[0])==null?void 0:g.delta;k!=null&&k.content&&(yield{content:k.content,done:!1,usage:null}),C.usage&&(m={prompt:C.usage.prompt_tokens,completion:C.usage.completion_tokens,total:C.usage.total_tokens})}catch(C){console.warn("OpenAI: Failed to parse chunk",C)}}}}catch(p){throw p instanceof b?p:b.streamInterrupted(p.message)}finally{c.releaseLock()}}}S(ae,"id","openai"),S(ae,"displayName","OpenAI");const Ne="https://api.anthropic.com/v1/messages",St="2023-06-01";class le extends R{static getModels(){return[{id:"claude-sonnet-4-20250514",name:"Claude Sonnet 4",description:"Latest, most capable"},{id:"claude-3-5-sonnet-20241022",name:"Claude 3.5 Sonnet",description:"Best balance"},{id:"claude-3-5-haiku-20241022",name:"Claude 3.5 Haiku",description:"Fast and affordable"},{id:"claude-3-opus-20240229",name:"Claude 3 Opus",description:"Most capable (legacy)"},{id:"claude-3-sonnet-20240229",name:"Claude 3 Sonnet",description:"Balanced (legacy)"},{id:"claude-3-haiku-20240307",name:"Claude 3 Haiku",description:"Fast (legacy)"}]}constructor(e,t="claude-3-5-sonnet-20241022"){super(e,t)}async*streamCompletion(e,t={}){var m;const{maxTokens:s=2048,temperature:i=.7,systemPrompt:o=null}=t,r={model:this.model,max_tokens:s,stream:!0,messages:[{role:"user",content:e}]};o&&(r.system=o),this.model.includes("thinking")||(r.temperature=i),console.log("Anthropic: Sending request to",Ne,"with model:",this.model);let a;try{a=await fetch(Ne,{method:"POST",headers:{"Content-Type":"application/json","x-api-key":this.apiKey,"anthropic-version":St,"anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify(r)})}catch(h){throw console.error("Anthropic: Network error",h),b.fromNetworkError(h)}if(console.log("Anthropic: Response status",a.status),!a.ok){let h=null;try{h=await a.json(),console.error("Anthropic: Error response",h)}catch{}throw b.fromHttpStatus(a.status,a.statusText,h)}const d=a.body.getReader(),c=new TextDecoder;let l="",u=null;try{for(;;){const{done:h,value:g}=await d.read();if(h){yield{content:"",done:!0,usage:u};break}l+=c.decode(g,{stream:!0});const p=l.split(`
`);l=p.pop()||"";for(const y of p){const x=y.trim();if(!(!x||!x.startsWith("data: ")))try{const v=JSON.parse(x.slice(6));if(v.type==="content_block_delta"){const w=v.delta;(w==null?void 0:w.type)==="text_delta"&&w.text&&(yield{content:w.text,done:!1,usage:null})}else v.type==="message_delta"?v.usage&&(u={prompt:v.usage.input_tokens||0,completion:v.usage.output_tokens||0,total:(v.usage.input_tokens||0)+(v.usage.output_tokens||0)}):v.type==="message_start"&&((m=v.message)!=null&&m.usage)&&(u={prompt:v.message.usage.input_tokens||0,completion:0,total:v.message.usage.input_tokens||0})}catch(v){console.warn("Anthropic: Failed to parse chunk",v)}}}}catch(h){throw h instanceof b?h:b.streamInterrupted(h.message)}finally{d.releaseLock()}}}S(le,"id","anthropic"),S(le,"displayName","Anthropic");const Pt="https://generativelanguage.googleapis.com/v1beta/models";class ce extends R{static getModels(){return[{id:"gemini-2.0-flash",name:"Gemini 2.0 Flash",description:"Latest, fastest"},{id:"gemini-1.5-pro",name:"Gemini 1.5 Pro",description:"Most capable"},{id:"gemini-1.5-flash",name:"Gemini 1.5 Flash",description:"Fast and efficient"},{id:"gemini-pro",name:"Gemini Pro",description:"Balanced (legacy)"}]}constructor(e,t="gemini-1.5-flash"){super(e,t)}async*streamCompletion(e,t={}){var x;const{maxTokens:s=2048,temperature:i=.7,systemPrompt:o=null}=t,r=[],a=o?{parts:[{text:o}]}:void 0;r.push({role:"user",parts:[{text:e}]});const d={contents:r,generationConfig:{maxOutputTokens:s,temperature:i}};a&&(d.systemInstruction=a);const c=`${Pt}/${this.model}:streamGenerateContent?alt=sse&key=${this.apiKey}`;console.log("Google: Sending request with model:",this.model);let l;try{l=await fetch(c,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)})}catch(v){throw console.error("Google: Network error",v),b.fromNetworkError(v)}if(console.log("Google: Response status",l.status),!l.ok){let v=null;try{v=await l.json(),console.error("Google: Error response",v)}catch{}throw b.fromHttpStatus(l.status,l.statusText,v)}const u=l.body.getReader(),m=new TextDecoder;let h="",g=null,p=0,y=0;try{for(;;){const{done:v,value:w}=await u.read();if(v){g={prompt:p,completion:y,total:p+y},yield{content:"",done:!0,usage:g};break}h+=m.decode(w,{stream:!0});const C=h.split(`
`);h=C.pop()||"";for(const k of C){const X=k.trim();if(!(!X||!X.startsWith("data: ")))try{const B=JSON.parse(X.slice(6)),Ze=B.candidates||[];for(const Qe of Ze){const et=((x=Qe.content)==null?void 0:x.parts)||[];for(const we of et)we.text&&(yield{content:we.text,done:!1,usage:null})}B.usageMetadata&&(p=B.usageMetadata.promptTokenCount||p,y=B.usageMetadata.candidatesTokenCount||y)}catch(B){console.warn("Google: Failed to parse chunk",B)}}}}catch(v){throw v instanceof b?v:b.streamInterrupted(v.message)}finally{u.releaseLock()}}}S(ce,"id","google"),S(ce,"displayName","Google");const Be="https://api.x.ai/v1/chat/completions";class de extends R{static getModels(){return[{id:"grok-3",name:"Grok 3",description:"Latest, most capable"},{id:"grok-3-fast",name:"Grok 3 Fast",description:"Faster responses"},{id:"grok-2",name:"Grok 2",description:"Balanced performance"},{id:"grok-2-mini",name:"Grok 2 Mini",description:"Fast and efficient"}]}constructor(e,t="grok-2"){super(e,t)}async*streamCompletion(e,t={}){var h,g;const{maxTokens:s=2048,temperature:i=.7,systemPrompt:o=null}=t,r=[];o&&r.push({role:"system",content:o}),r.push({role:"user",content:e});const a={model:this.model,messages:r,max_tokens:s,temperature:i,stream:!0};console.log("X/Grok: Sending request to",Be,"with model:",this.model);let d;try{d=await fetch(Be,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.apiKey}`},body:JSON.stringify(a)})}catch(p){throw console.error("X/Grok: Network error",p),b.fromNetworkError(p)}if(console.log("X/Grok: Response status",d.status),!d.ok){let p=null;try{p=await d.json(),console.error("X/Grok: Error response",p)}catch{}throw b.fromHttpStatus(d.status,d.statusText,p)}const c=d.body.getReader(),l=new TextDecoder;let u="",m=null;try{for(;;){const{done:p,value:y}=await c.read();if(p){yield{content:"",done:!0,usage:m};break}u+=l.decode(y,{stream:!0});const x=u.split(`
`);u=x.pop()||"";for(const v of x){const w=v.trim();if(!(!w||w==="data: [DONE]")&&w.startsWith("data: "))try{const C=JSON.parse(w.slice(6)),k=(g=(h=C.choices)==null?void 0:h[0])==null?void 0:g.delta;k!=null&&k.content&&(yield{content:k.content,done:!1,usage:null}),C.usage&&(m={prompt:C.usage.prompt_tokens,completion:C.usage.completion_tokens,total:C.usage.total_tokens})}catch(C){console.warn("X/Grok: Failed to parse chunk",C)}}}}catch(p){throw p instanceof b?p:b.streamInterrupted(p.message)}finally{c.releaseLock()}}}S(de,"id","x"),S(de,"displayName","X (Grok)");const K={OPENAI:"openai",ANTHROPIC:"anthropic",GOOGLE:"google",X:"x"},ye={[K.OPENAI]:ae,[K.ANTHROPIC]:le,[K.GOOGLE]:ce,[K.X]:de};function Mt(){return Object.values(ye).map(n=>({id:n.id,name:n.displayName,models:n.getModels(),defaultModel:n.getDefaultModel()}))}function We(n,e,t=null){const s=n==null?void 0:n.toLowerCase(),i=ye[s];if(!i)throw new b(`Unknown provider type: ${n}`,E.UNKNOWN,!1);const o=t||i.getDefaultModel();return new i(e,o)}function Ie(n){const e=ye[n==null?void 0:n.toLowerCase()];return e?e.getModels():[]}const ee=Mt();class At extends Ye{constructor(){super({title:"Settings"}),this.settings=N(),this.apiKeyInputs={}}createContent(){const e=document.createElement("div");return e.style.cssText="display: flex; flex-direction: column; gap: var(--pc-space-6);",e.appendChild(this.createThemeSection()),e.appendChild(this.createCoachLLMSection()),e.appendChild(this.createTestLLMSection()),e.appendChild(this.createApiKeysSection()),e}createThemeSection(){const e=document.createElement("div"),t=document.createElement("h3");t.className="text-title",t.style.cssText="margin-bottom: var(--pc-space-3);",t.textContent="Theme",e.appendChild(t);const s=document.createElement("div");return s.style.cssText="display: flex; gap: var(--pc-space-3);",["system","light","dark"].forEach(i=>{const o=document.createElement("label");o.style.cssText=`
        display: flex;
        align-items: center;
        gap: var(--pc-space-2);
        cursor: pointer;
        padding: var(--pc-space-2);
      `;const r=document.createElement("input");r.type="radio",r.name="theme",r.value=i,r.checked=this.settings.theme===i,r.addEventListener("change",()=>{this.settings.theme=i,yt(i)});const a=document.createElement("span");a.className="text-body",a.textContent=i.charAt(0).toUpperCase()+i.slice(1),o.appendChild(r),o.appendChild(a),s.appendChild(o)}),e.appendChild(s),e}createCoachLLMSection(){const e=document.createElement("div"),t=document.createElement("h3");t.className="text-title",t.style.cssText="margin-bottom: var(--pc-space-3);",t.textContent="Coach LLM",e.appendChild(t);const s=document.createElement("p");s.className="text-body",s.style.cssText="color: var(--pc-on-surface-variant); margin-bottom: var(--pc-space-2); font-size: 0.85rem;",s.textContent="Model used for prompt evaluation and feedback.",e.appendChild(s);const i=document.createElement("div");i.style.cssText="display: flex; gap: var(--pc-space-3); flex-wrap: wrap;";const o=document.createElement("div");o.style.cssText="flex: 1; min-width: 140px;";const r=document.createElement("label");r.className="text-label",r.style.cssText="display: block; margin-bottom: var(--pc-space-1); color: var(--pc-on-surface-variant);",r.textContent="Provider",o.appendChild(r),this.coachProviderSelect=document.createElement("select"),this.coachProviderSelect.className="input",this.coachProviderSelect.style.cssText="width: 100%;",ee.forEach(c=>{const l=document.createElement("option");l.value=c.id,l.textContent=c.name,this.coachProviderSelect.appendChild(l)}),this.coachProviderSelect.value=this.settings.coachProvider||"openai",this.coachProviderSelect.addEventListener("change",()=>{this.settings.coachProvider=this.coachProviderSelect.value,this.updateCoachModelOptions()}),o.appendChild(this.coachProviderSelect),i.appendChild(o);const a=document.createElement("div");a.style.cssText="flex: 2; min-width: 200px;";const d=document.createElement("label");return d.className="text-label",d.style.cssText="display: block; margin-bottom: var(--pc-space-1); color: var(--pc-on-surface-variant);",d.textContent="Model",a.appendChild(d),this.coachModelSelect=document.createElement("select"),this.coachModelSelect.className="input",this.coachModelSelect.style.cssText="width: 100%;",this.coachModelSelect.addEventListener("change",()=>{this.settings.coachModel=this.coachModelSelect.value}),a.appendChild(this.coachModelSelect),i.appendChild(a),e.appendChild(i),this.updateCoachModelOptions(),e}updateCoachModelOptions(){const e=this.coachProviderSelect.value,t=Ie(e);this.coachModelSelect.innerHTML="",t.forEach(i=>{const o=document.createElement("option");o.value=i.id,o.textContent=i.name,i.description&&(o.title=i.description),this.coachModelSelect.appendChild(o)});const s=t.map(i=>i.id);this.settings.coachModel&&s.includes(this.settings.coachModel)?this.coachModelSelect.value=this.settings.coachModel:t.length>0&&(this.coachModelSelect.value=t[0].id,this.settings.coachModel=t[0].id)}createTestLLMSection(){const e=document.createElement("div"),t=document.createElement("h3");t.className="text-title",t.style.cssText="margin-bottom: var(--pc-space-3);",t.textContent="Test LLM",e.appendChild(t);const s=document.createElement("div");s.style.cssText="display: flex; gap: var(--pc-space-3); flex-wrap: wrap;";const i=document.createElement("div");i.style.cssText="flex: 1; min-width: 140px;";const o=document.createElement("label");o.className="text-label",o.style.cssText="display: block; margin-bottom: var(--pc-space-1); color: var(--pc-on-surface-variant);",o.textContent="Provider",i.appendChild(o),this.testProviderSelect=document.createElement("select"),this.testProviderSelect.className="input",this.testProviderSelect.style.cssText="width: 100%;",ee.forEach(d=>{const c=document.createElement("option");c.value=d.id,c.textContent=d.name,this.testProviderSelect.appendChild(c)}),this.testProviderSelect.value=this.settings.testProvider||"openai",this.testProviderSelect.addEventListener("change",()=>{this.settings.testProvider=this.testProviderSelect.value,this.updateModelOptions()}),i.appendChild(this.testProviderSelect),s.appendChild(i);const r=document.createElement("div");r.style.cssText="flex: 2; min-width: 200px;";const a=document.createElement("label");return a.className="text-label",a.style.cssText="display: block; margin-bottom: var(--pc-space-1); color: var(--pc-on-surface-variant);",a.textContent="Model",r.appendChild(a),this.testModelSelect=document.createElement("select"),this.testModelSelect.className="input",this.testModelSelect.style.cssText="width: 100%;",this.testModelSelect.addEventListener("change",()=>{this.settings.testModel=this.testModelSelect.value}),r.appendChild(this.testModelSelect),s.appendChild(r),e.appendChild(s),this.updateModelOptions(),e}updateModelOptions(){const e=this.testProviderSelect.value,t=Ie(e);this.testModelSelect.innerHTML="",t.forEach(i=>{const o=document.createElement("option");o.value=i.id,o.textContent=i.name,i.description&&(o.title=i.description),this.testModelSelect.appendChild(o)});const s=t.map(i=>i.id);this.settings.testModel&&s.includes(this.settings.testModel)?this.testModelSelect.value=this.settings.testModel:t.length>0&&(this.testModelSelect.value=t[0].id,this.settings.testModel=t[0].id)}createApiKeysSection(){const e=document.createElement("div"),t=document.createElement("div");t.style.cssText="margin-bottom: var(--pc-space-3);";const s=document.createElement("h3");s.className="text-title",s.textContent="API Keys",t.appendChild(s);const i=document.createElement("p");i.className="text-body",i.style.cssText="color: var(--pc-on-surface-variant); margin-top: var(--pc-space-1);",i.textContent="⚠️ Keys are stored locally on your device.",t.appendChild(i),e.appendChild(t);const o=document.createElement("div");return o.style.cssText="display: flex; flex-direction: column; gap: var(--pc-space-3);",ee.forEach(r=>{const a=document.createElement("div"),d=document.createElement("label");d.className="text-label",d.style.cssText="display: block; margin-bottom: var(--pc-space-1); color: var(--pc-on-surface-variant);",d.textContent=r.name,a.appendChild(d);const c=document.createElement("input");c.type="password",c.className="input",c.placeholder=`Enter ${r.name} API key`,c.value=this.settings.apiKeys[r.id]||"",c.autocomplete="off",c.addEventListener("input",l=>{this.settings.apiKeys[r.id]=l.target.value.trim()||null}),this.apiKeyInputs[r.id]=c,a.appendChild(c),o.appendChild(a)}),e.appendChild(o),e}show(){this.render(),this.setContent(this.createContent()),this.addAction("Cancel",()=>this.close(),"text"),this.addAction("Save",()=>this.save(),"filled"),super.show()}save(){Ue(this.settings),f("Settings saved"),this.close()}}let L=null,pe=[];function Lt(){window.addEventListener("beforeinstallprompt",n=>{n.preventDefault(),L=n,pe.forEach(e=>e(!0))}),window.addEventListener("appinstalled",()=>{L=null,pe.forEach(n=>n(!1))})}function be(){return L!==null}async function Nt(){if(!L)return!1;L.prompt();const{outcome:n}=await L.userChoice;return L=null,n==="accepted"}function Bt(n){pe.push(n),n(be())}class It{constructor(e={}){this.onClose=e.onClose||null,this.element=null,this.backdrop=null,this.isOpen=!1}render(){const e=document.createElement("div");e.className="menu-container",e.style.cssText="position: fixed; inset: 0; z-index: var(--pc-z-overlay); pointer-events: none;",this.backdrop=document.createElement("div"),this.backdrop.className="menu-backdrop",this.backdrop.style.cssText=`
      position: absolute;
      inset: 0;
    ...
    `;const s=document.createElement("h2");s.className="text-title-lg",s.textContent="Prompt Coach",t.appendChild(s),this.element.appendChild(t);const i=document.createElement("ul");return i.style.cssText="list-style: none; padding: var(--pc-space-2) 0; flex: 1;",[{label:"New Session",icon:"add",action:()=>Me()},{label:"History",icon:"history",action:()=>Me()},{label:"Settings",icon:"settings",action:()=>this.openSettings()},{label:"Install App",icon:"install",action:()=>this.handleInstall(),id:"install-item",hidden:!be()}].forEach(r=>{const a=document.createElement("li");r.id&&(a.id=r.id),r.hidden&&(a.style.display="none");const d=document.createElement("button");d.className="menu-item",d.style.cssText=`
        display: flex;
        align-items: center;
        gap: var(--pc-space-4);
        width: 100%;
    ...
    `;const i=e.overall,o=i>=80?"var(--pc-primary)":i>=60?"#F59E0B":"#EF4444",r=document.createElement("div");r.style.cssText=`
      ...
      font-weight: 600;
      font-size: 1.1rem;
    `,r.textContent=i,s.appendChild(r);const a=document.createElement("div");a.style.cssText="flex: 1;";const d=document.createElement("div");d.className="text-label",d.style.cssText="color: var(--pc-on-surface-variant); margin-bottom: 2px;",d.textContent=t?"🎉 Coaching Complete!":"Overall Assessment",a.appendChild(d);const c=document.createElement("div");c.className="text-body",c.textContent=e.description,a.appendChild(c),s.appendChild(a),this.contentEl.appendChild(s);const l=document.createElement("div");if(l.className="feedback-text",l.style.cssText=`
      line-height: 1.6;
      color: var(--pc-on-surface);
    `,l.textContent=e.feedback,this.contentEl.appendChild(l),e.scores&&e.scores.length>0){const u=document.createElement("details");u.style.cssText="margin-top: var(--pc-space-4);";const m=document.createElement("summary");m.style.cssText=`
        cursor: pointer;
        color: var(--pc-primary);
        font-weight: 500;
        padding: var(--pc-space-2) 0;
      `,m.textContent="View all principle scores",u.appendChild(m);const h=document.createElement("div");h.style.cssText=`
        display: flex;
        flex-direction: column;
        gap: var(--pc-space-2);
      ...
      `,e.scores.forEach(g=>{const p=document.createElement("div");p.style.cssText=`
        ...
        `,x.textContent=g.score,p.appendChild(x),h.appendChild(p)}),u.appendChild(h),this.contentEl.appendChild(u)}}togglePin(){this.setPinned(!this.isPinned)}setPinned(e){this.isPinned=e,this.pinBtn.style.color=e?"var(--pc-primary)":"var(--pc-on-surface-variant)",e?(this.element.style.position="relative",this.element.style.transform="none",this.element.style.maxHeight=`${this.ratio*100}vh`,this.element.style.borderBottom="3px solid var(--pc-primary)",this.backdrop.style.display="none",this.divider&&(this.divider.style.display="block")):(this.element.style.position="fixed",this.element.style.transform=this.isVisible?"translateY(0)":"translateY(-100%)",this.element.style.maxHeight="60vh",this.element.style.borderBottom="1px solid var(--pc-outline-variant)",this.backdrop.style.display="block",this.divider&&(this.divider.style.display="none")),I("feedbackPanelPinned",e),this.onPin&&this.onPin(e)}setRatio(e){this.ratio=Math.max(.15,Math.min(.7,e)),this.isPinned&&(this.element.style.maxHeight=`${this.ratio*100}vh`),I("feedbackPanelRatio",this.ratio)}loadPersistedState(){const e=W();this.isPinned=e.feedbackPanelPinned||!1,this.ratio=e.feedbackPanelRatio||.3,this.isPinned&&(this.pinBtn.style.color="var(--pc-primary)")}getBackdrop(){return this.backdrop}}function H(n){const e={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"};return n.replace(/[&<>"']/g,t=>e[t])}function ue(n){if(!n||typeof n!="string")return"";const e=n.split(`
...
`)}),s=[]),r&&a.length>0&&(t.push({type:"list",items:a}),a=[],r=!1),t.push({type:"hr"});continue}const g=l.match(/^>\s+(.+)$/);if(g){s.length>0&&(t.push({type:"paragraph",content:s.join(`
`)}),s=[]),r&&a.length>0&&(t.push({type:"list",items:a}),a=[],r=!1),t.push({type:"blockquote",content:g[1]});continue}if(l.trim()===""){r&&a.length>0&&(t.push({type:"list",items:a}),a=[],r=!1),s.length>0&&(t.push({type:"paragraph",content:s.join(`
...
`)}),t.map(c=>{switch(c.type){case"header":return`<h${c.level}>${j(H(c.content))}</h${c.level}>`;case"paragraph":const l=j(H(c.content));return l?`<p>${l}</p>`:"";case"list":return`<ul>${c.items.map(u=>`<li>${j(H(u))}</li>`).join("")}</ul>`;case"code":return`<pre><code class="language-${c.lang}">${H(c.content)}</code></pre>`;case"blockquote":return`<blockquote>${j(H(c.content))}</blockquote>`;case"hr":return"<hr>";default:return""}}).filter(Boolean).join("")}function j(n){let e=n;return e=e.replace(/`([^`]+)`/g,"<code>$1</code>"),e=e.replace(/\*\*\*(.+?)\*\*\*/g,"<strong><em>$1</em></strong>"),e=e.replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>"),e=e.replace(/\*(.+?)\*/g,"<em>$1</em>"),e=e.replace(/___(.+?)___/g,"<strong><em>$1</em></strong>"),e=e.replace(/__(.+?)__/g,"<strong>$1</strong>"),e=e.replace(/_(.+?)_/g,"<em>$1</em>"),e=e.replace(/\[([^\]]+)\]\(([^)]+)\)/g,'<a href="$2" target="_blank" rel="noopener">$1</a>'),e}const Ht="modulepreload",Dt=function(n,e){return new URL(n,e).href},Re={},He=function(e,t,s){let i=Promise.resolve();if(t&&t.length>0){const r=document.getElementsByTagName("link"),a=document.querySelector("meta[property=csp-nonce]"),d=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));i=Promise.allSettled(t.map(c=>{if(c=Dt(c,s),c in Re)return;Re[c]=!0;const l=c.endsWith(".css"),u=l?'[rel="stylesheet"]':"";if(!!s)for(let g=r.length-1;g>=0;g--){const p=r[g];if(p.href===c&&(!l||p.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${c}"]${u}`))return;const h=document.createElement("link");if(h.rel=l?"stylesheet":Ht,l||(h.as="script"),h.crossOrigin="",h.href=c,d&&h.setAttribute("nonce",d),document.head.appendChild(h),l)return new Promise((g,p)=>{h.addEventListener("load",g),h.addEventListener("error",()=>p(new Error(`Unable to preload CSS for ${c}`)))})}))}function o(r){const a=new Event("vite:preloadError",{cancelable:!0});if(a.payload=r,window.dispatchEvent(a),!a.defaultPrevented)throw r}return i.then(r=>{for(const a of r||[])a.status==="rejected"&&o(a.reason);return e().catch(o)})};function Ot(n,e=null){const t=[];if(!n||typeof n!="object")return{valid:!1,errors:["Response must be a JSON object"]};if(n.scores?Array.isArray(n.scores)?n.scores.length===0&&t.push("scores array must not be empty"):t.push("scores must be an array"):t.push("Missing required field: scores"),(n.overall===void 0||n.overall===null)&&t.push("Missing required field: overall"),n.description||t.push("Missing required field: description"),n.feedback||t.push("Missing required field: feedback"),t.length>0)return{valid:!1,errors:t};if(typeof n.overall!="number"&&t.push("overall must be a number"),typeof n.description!="string"&&t.push("description must be a string"),typeof n.feedback!="string"&&t.push("feedback must be a string"),n.scores.forEach((s,i)=>{(!s.principle||typeof s.principle!="string")&&t.push(`scores[${i}].principle must be a non-empty string`),(s.score===void 0||typeof s.score!="number")&&t.push(`scores[${i}].score must be a number`),(!s.reason||typeof s.reason!="string")&&t.push(`scores[${i}].reason must be a non-empty string`)}),typeof n.overall=="number"&&(n.overall<0||n.overall>100)&&t.push("overall must be between 0 and 100"),n.scores.forEach((s,i)=>{typeof s.score=="number"&&(s.score<0||s.score>100)&&t.push(`scores[${i}].score must be between 0 and 100`)}),typeof n.description=="string"&&n.description.length>100&&t.push("description must be 100 characters or less"),typeof n.description=="string"&&n.description.trim()===""&&t.push("description must not be empty"),typeof n.feedback=="string"&&n.feedback.trim()===""&&t.push("feedback must not be empty"),n.complete===void 0&&(n.complete=!1),!n.complete&&!n.targetPrinciple&&console.warn("targetPrinciple missing when not complete - using first low-scoring principle"),n.targetPrinciple!==void 0&&typeof n.targetPrinciple!="string"&&t.push("targetPrinciple must be a string"),n.complete!==void 0&&typeof n.complete!="boolean"&&t.push("complete must be a boolean"),e&&e.principles){const s=e.principles.map(i=>i.id);n.scores.forEach((i,o)=>{i.principle&&!s.includes(i.principle)&&console.warn(`scores[${o}].principle "${i.principle}" not in methodology`)}),n.targetPrinciple&&!s.includes(n.targetPrinciple)&&console.warn(`targetPrinciple "${n.targetPrinciple}" not in methodology`)}return{valid:t.length===0,errors:t}}let D=null,se=null;class _t{constructor(e,t,s){this.provider=We(e,t,s),this.methodology=null,this.systemPrompt=null}async loadMethodology(){if(this.methodology)return this.methodology;if(!D)try{D=(await He(()=>import("./principles-Nr9ahyFn.js"),[],import.meta.url)).default}catch(e){throw console.error("Failed to load principles.json:",e),new Error("Coaching methodology not configured")}if(!D||!D.principles)throw new Error("Coaching methodology not configured");return this.methodology=D,this.methodology}async buildSystemPrompt(){if(this.systemPrompt)return this.systemPrompt;const e=await this.loadMethodology();if(!se)try{se=(await He(()=>import("./system-prompt-Dr6qRAWU.js"),[],import.meta.url)).default}catch(s){throw console.error("Failed to load system-prompt.md:",s),new Error("Coaching methodology not configured")}const t=e.principles.map((s,i)=>`${i+1}. **${s.name}** (weight: ${s.weight}): ${s.description}`).join(`
`);return this.systemPrompt=se.replace("{{PRINCIPLES}}",t),this.systemPrompt}async evaluate(e,t=[],s={}){const{attachmentContext:i="",attachmentsTruncated:o=!1}=s,r=await this.buildSystemPrompt(),a=performance.now();let d=e.length;i&&(d+=i.length),await Ke(this.provider.getProviderName(),this.provider.getModelName(),d);let c="";try{let l=e;i&&(l=`[ATTACHED CONTEXT${o?" (truncated)":""}]
${i}
[END ATTACHED CONTEXT]

${e}`),t.length>0&&(l=`${l}

[SKIPPED_PRINCIPLES: ${t.join(", ")}]`);for await(const g of this.provider.streamCompletion(l,{maxTokens:1024,temperature:.3,systemPrompt:r}))g.content&&(c+=g.content);const u=Math.round(performance.now()-a),m=this.parseResponse(c),h=Ot(m,this.methodology);if(!h.valid)throw new Error(`Invalid response: ${h.errors.join(", ")}`);return await je(this.provider.getProviderName(),this.provider.getModelName(),c.length,null,u),m}catch(l){const u=Math.round(performance.now()-a);throw await ne(this.provider.getProviderName(),this.provider.getModelName(),l.message||"Unknown error",u),l}}async evaluateWithRetry(e,t=[],s={}){try{return await this.evaluate(e,t,s)}catch(i){if(i instanceof b)throw i;console.warn("Coach: First attempt failed, retrying...",i.message);const o=`${e}

IMPORTANT: Please respond with valid JSON only, matching the exact schema specified.`;try{return await this.evaluate(o,t,s)}catch(r){throw console.error("Coach: Retry also failed",r.message),new Error("Coach couldn't evaluate. Try again.")}}}parseResponse(e){let t=e.trim();const s=t.match(/```(?:json)?\s*([\s\S]*?)```/);s&&(t=s[1].trim());const i=t.match(/\{[\s\S]*\}/);i&&(t=i[0]);try{return JSON.parse(t)}catch(o){throw new Error(`Failed to parse JSON response: ${o.message}`)}}getProviderName(){return this.provider.getProviderName()}getModelName(){return this.provider.getModelName()}}const zt=[{combo:"ctrl+enter",action:"test",enabled:!0},{combo:"ctrl+shift+enter",action:"coach",enabled:!0},{combo:"ctrl+z",action:"undo",enabled:!0},{combo:"ctrl+y",action:"redo",enabled:!0}];class $t{constructor(e=zt){this.shortcuts=new Map,e.forEach(t=>{t.enabled&&this.shortcuts.set(t.combo.toLowerCase(),t.action)}),this.enabled=!0,this.actionDispatcher=null,this.isActiveCheck=null,this.boundHandler=null}init(e,t){this.actionDispatcher=e,this.isActiveCheck=t,this.boundHandler=this.handleKeyDown.bind(this),document.addEventListener("keydown",this.boundHandler)}handleKeyDown(e){if(!this.enabled||!this.isActiveCheck||!this.isActiveCheck())return;const t=this.buildCombo(e),s=this.shortcuts.get(t);s&&(e.preventDefault(),e.stopPropagation(),this.actionDispatcher&&this.actionDispatcher(s))}buildCombo(e){const t=[];(e.ctrlKey||e.metaKey)&&t.push("ctrl"),e.shiftKey&&t.push("shift"),e.altKey&&t.push("alt");let s=e.key.toLowerCase();return s===" "&&(s="space"),s==="escape"&&(s="esc"),t.push(s),t.join("+")}setEnabled(e){this.enabled=e}addShortcut(e,t){this.shortcuts.set(e.toLowerCase(),t)}removeShortcut(e){this.shortcuts.delete(e.toLowerCase())}getShortcuts(){return Array.from(this.shortcuts.entries()).map(([e,t])=>({combo:e,action:t}))}destroy(){this.boundHandler&&(document.removeEventListener("keydown",this.boundHandler),this.boundHandler=null),this.actionDispatcher=null,this.isActiveCheck=null}}const De="promptHistory",Oe="prompt_history";class Ft{constructor(e=50){this.maxSize=e,this.entries=[],this.position=-1,this.debounceTimer=null,this.loaded=!1}async load(){try{const e=await fe(De,Oe);e&&(this.entries=e.entries||[],this.position=e.position??-1,this.maxSize=e.maxSize||this.maxSize),this.loaded=!0}catch(e){console.warn("Failed to load prompt history:",e),this.loaded=!0}}push(e){var t;this.position>=0&&((t=this.entries[this.position])==null?void 0:t.text)===e||(this.entries=this.entries.slice(0,this.position+1),this.entries.push({text:e,timestamp:Date.now()}),this.entries.length>this.maxSize&&this.entries.shift(),this.position=this.entries.length-1,this.persist())}pushDebounced(e,t=500){this.debounceTimer&&clearTimeout(this.debounceTimer),this.debounceTimer=setTimeout(()=>{this.push(e),this.debounceTimer=null},t)}cancelDebounce(){this.debounceTimer&&(clearTimeout(this.debounceTimer),this.debounceTimer=null)}undo(){return this.canUndo()?(this.position--,this.persist(),this.entries[this.position].text):null}redo(){return this.canRedo()?(this.position++,this.persist(),this.entries[this.position].text):null}canUndo(){return this.position>0}canRedo(){return this.position<this.entries.length-1}getCurrent(){return this.position<0||this.position>=this.entries.length?null:this.entries[this.position].text}get length(){return this.entries.length}async persist(){try{await z(De,{id:Oe,entries:this.entries,position:this.position,maxSize:this.maxSize})}catch(e){console.warn("Failed to persist prompt history:",e)}}async clear(){this.entries=[],this.position=-1,this.cancelDebounce(),await this.persist()}}const me={"text/plain":".txt","text/markdown":".md","application/json":".json",".txt":"text/plain",".md":"text/markdown",".json":"application/json"},Kt=[".txt",".md",".json"],Xe=102400,jt=5e4,O={UNSUPPORTED_TYPE:"UNSUPPORTED_TYPE",SIZE_EXCEEDED:"SIZE_EXCEEDED",EMPTY_FILE:"EMPTY_FILE",READ_ERROR:"READ_ERROR"};function xe(n){if(!n)return"";const e=n.lastIndexOf(".");return e===-1?"":n.slice(e).toLowerCase()}function qt(n){if(n.type&&me[n.type])return!0;const e=xe(n.name);return Kt.includes(e)}function Vt(n){if(n.type&&me[n.type])return n.type;const e=xe(n.name);return me[e]||e}function Ut(n,e=Xe){if(!qt(n))return{valid:!1,error:`Unsupported file type: ${xe(n.name)||n.type||"unknown"}. Supported: .txt, .md, .json`,errorCode:O.UNSUPPORTED_TYPE};if(n.size>e){const t=Math.round(e/1024);return{valid:!1,error:`File too large: ${Math.round(n.size/1024)}KB (limit: ${t}KB)`,errorCode:O.SIZE_EXCEEDED}}return n.size===0?{valid:!1,error:"File is empty",errorCode:O.EMPTY_FILE}:{valid:!0}}function Gt(n){return new Promise((e,t)=>{const s=new FileReader;s.onload=()=>e(s.result),s.onerror=()=>t(new Error(`Failed to read file: ${n.name}`)),s.readAsText(n)})}function Yt(n){return n<1024?`${n} B`:n<1024*1024?`${(n/1024).toFixed(1)} KB`:`${(n/(1024*1024)).toFixed(1)} MB`}class U extends Error{constructor(e,t,s){super(e),this.name="AttachmentError",this.code=t,this.filename=s}}async function Wt(n,e=Xe){const t=Ut(n,e);if(!t.valid)throw new U(t.error,t.errorCode,n.name);let s;try{s=await Gt(n)}catch{throw new U(`Failed to read file: ${n.name}`,O.READ_ERROR,n.name)}if(!s||s.trim()==="")throw new U("File is empty",O.EMPTY_FILE,n.name);const i={id:crypto.randomUUID(),filename:n.name,content:s,size:n.size,type:Vt(n),addedAt:new Date().toISOString()};return await T(),await z("attachments",i),i}async function Je(){return await T(),(await ge("attachments")).sort((e,t)=>new Date(t.addedAt).getTime()-new Date(e.addedAt).getTime())}async function Xt(n){return await T(),await fe("attachments",n)?(await at("attachments",n),!0):!1}async function Jt(){await T();const e=(await ge("attachments")).length;return await lt("attachments"),e}async function _e(n=jt){const e=await Je();if(e.length===0)return{context:"",truncated:!1,count:0};const t=[];let s=0,i=!1;for(const o of e){const r=`--- Attached File: ${o.filename} ---`,a=`--- End of ${o.filename} ---`,d=r.length+a.length+4;if(s+d+o.content.length>n){const c=n-s-d-50;if(c>100){const l=o.content.slice(0,c);t.push(`${r}
${l}
[... content truncated ...]
...
`),truncated:i,count:e.length}}class Zt{constructor(){this.element=null,this.textarea=null,this.preview=null,this.isPreviewMode=!1,this.promptText="",this.initialized=!1,this.isTestRunning=!1,this.isCoachRunning=!1,this.onSwitchToResults=null,this.resultsTab=null,this.feedbackPanel=null,this.onShowFeedback=null,this.keyboardHandler=null,this.promptHistory=new Ft(50)}render(){if(this.element)return this.element;this.element=document.createElement("div"),this.element.className="prompt-tab",this.element.setAttribute("role","tabpanel"),this.element.setAttribute("id","prompt-panel"),this.element.style.cssText=`
      display: flex;
      flex-direction: column;
      height: 100%;
      padding: var(--pc-space-4);
    `;const e=this.createToolbar();this.element.appendChild(e);const t=document.createElement("div");return t.style.cssText="flex: 1; position: relative; min-height: 0;",this.textarea=document.createElement("textarea"),this.textarea.className="input textarea",this.textarea.placeholder="Your prompt here...",this.textarea.style.cssText=`
      ...
      font-size: 1rem;
      line-height: 1.5;
    `,this.textarea.addEventListener("input",s=>{this.promptText=s.target.value,Q(this.promptText),this.promptHistory.pushDebounced(this.promptText,1e3)}),t.appendChild(this.textarea),this.preview=document.createElement("div"),this.preview.className="prompt-preview",this.preview.style.cssText=`
      position: absolute;
      inset: 0;
      padding: var(--pc-space-4);
      background-color: var(--pc-surface);
    ...
    `,this.testBtn.addEventListener("click",()=>this.runTest()),e.appendChild(this.testBtn),this.coachBtn=document.createElement("button"),this.coachBtn.className="btn btn-text-compact",this.coachBtn.title="Get coaching feedback (Ctrl+Shift+Enter)",this.coachBtn.innerHTML=`
      ...
      <span>Review</span>
    `,this.coachBtn.addEventListener("click",()=>this.runCoach()),e.appendChild(this.coachBtn),e}setMode(e){this.isPreviewMode!==e&&(this.isPreviewMode=e,this.isPreviewMode?(this.textarea.style.display="none",this.preview.style.display="block",this.preview.innerHTML=ue(this.promptText)||'<p style="color: var(--pc-on-surface-variant);">Nothing to preview</p>',this.applyMarkdownStyles(this.preview),this.editBtn.style.background="transparent",this.editBtn.style.color="var(--pc-on-surface)",this.previewBtn.style.background="var(--pc-primary)",this.previewBtn.style.color="var(--pc-on-primary)"):(this.textarea.style.display="block",this.preview.style.display="none",this.textarea.focus(),this.editBtn.style.background="var(--pc-primary)",this.editBtn.style.color="var(--pc-on-primary)",this.previewBtn.style.background="transparent",this.previewBtn.style.color="var(--pc-on-surface)"))}async copyPrompt(){if(!this.promptText.trim()){f("Nothing to copy");return}try{await navigator.clipboard.writeText(this.promptText),f("Copied to clipboard")}catch{f("Failed to copy")}}async loadPrompt(){try{const e=await M();this.promptText=e.promptText||"",this.textarea&&(this.textarea.value=this.promptText),await this.promptHistory.load(),this.promptText&&this.promptHistory.push(this.promptText),this.initKeyboardHandler()}catch(e){console.error("Failed to load prompt:",e)}}initKeyboardHandler(){this.keyboardHandler||(this.keyboardHandler=new $t,this.keyboardHandler.init(e=>this.handleKeyboardAction(e),()=>this.isKeyboardActive()))}isKeyboardActive(){return this.element&&this.element.offsetParent!==null&&!this.isPreviewMode}handleKeyboardAction(e){switch(e){case"test":this.runTest();break;case"coach":this.runCoach();break;case"undo":this.handleUndo();break;case"redo":this.handleRedo();break;default:console.warn("Unknown keyboard action:",e)}}handleUndo(){const e=this.promptHistory.undo();e!==null&&(this.promptText=e,this.textarea&&(this.textarea.value=e),Q(e),f("Undo"))}handleRedo(){const e=this.promptHistory.redo();e!==null&&(this.promptText=e,this.textarea&&(this.textarea.value=e),Q(e),f("Redo"))}async runTest(){var m,h,g;if(this.isTestRunning){f("Test already in progress");return}const e=this.promptText.trim();if(!e){f("Please enter a prompt first");return}if(!oe()){f("Please configure an API key in Settings");return}const t=N(),s=t.testProvider||"openai",i=t.apiKeys[s],o=t.testModel||null;if(!i){f(`No API key configured for ${s}`);return}let r;try{r=We(s,i,o)}catch(p){f(p.message||"Failed to create provider");return}let a=e,d=null;try{const{context:p,truncated:y,count:x}=await _e();p&&(a=`${p}

---

${e}`,d={count:x,truncated:y},y&&f("Attachments truncated due to size limit","warning"))}catch(p){console.warn("Failed to get attachment context:",p)}this.isTestRunning=!0,this.testBtn.disabled=!0,this.setButtonLoading(this.testBtn,!0);const c=mt({promptSnapshot:e,provider:r.getProviderName(),model:r.getModelName(),status:"streaming",attachmentCount:(d==null?void 0:d.count)||0});this.onSwitchToResults&&this.onSwitchToResults();const l=(m=this.resultsTab)==null?void 0:m.addResult(c);await ht(c);const u=performance.now();await Ke(r.getProviderName(),r.getModelName(),a.length);try{for await(const p of r.streamCompletion(a))if(p.content&&(l==null||l.appendChunk(p.content)),p.done&&p.usage){const y=Math.round(performance.now()-u);l==null||l.complete(p.usage,y),await Te(c.id,{responseText:(l==null?void 0:l.getResult().responseText)||"",tokens:p.usage,durationMs:y,status:"complete"}),await je(r.getProviderName(),r.getModelName(),((h=l==null?void 0:l.getResult().responseText)==null?void 0:h.length)||0,p.usage,y),(g=this.resultsTab)==null||g.updateSessionStats()}}catch(p){const y=Math.round(performance.now()-u),x=p instanceof b?p.userMessage:p.message||"Unknown error";l==null||l.setError(x,y),await Te(c.id,{responseText:(l==null?void 0:l.getResult().responseText)||"",error:x,durationMs:y,status:"error"}),await ne(r.getProviderName(),r.getModelName(),x,y),f(x)}finally{this.isTestRunning=!1,this.testBtn.disabled=!1,this.setButtonLoading(this.testBtn,!1)}}setOnSwitchToResults(e){this.onSwitchToResults=e}setResultsTab(e){this.resultsTab=e}setOnShowFeedback(e){this.onShowFeedback=e}async runCoach(){if(this.isCoachRunning){f("Coach evaluation in progress");return}const e=this.promptText.trim();if(!e){f("Please enter a prompt first");return}if(!oe()){f("Please configure an API key in Settings");return}const t=N(),s=t.coachProvider||"openai",i=t.apiKeys[s],o=t.coachModel||null;if(!i){f(`No API key configured for ${s}`);return}let r="",a=!1;try{const{context:c,truncated:l}=await _e();r=c,a=l,l&&f("Attachments truncated due to size limit","warning")}catch(c){console.warn("Failed to get attachment context:",c)}this.isCoachRunning=!0,this.coachBtn.disabled=!0,this.setButtonLoading(this.coachBtn,!0);const d=performance.now();try{const c=new _t(s,i,o),l=he.get(),u=await c.evaluateWithRetry(e,l,{attachmentContext:r,attachmentsTruncated:a}),m=Math.round(performance.now()-d),h=ft({promptSnapshot:e,scores:u.scores,overall:u.overall,description:u.description,feedback:u.feedback,provider:c.getProviderName(),model:c.getModelName(),durationMs:m,targetPrinciple:u.targetPrinciple,complete:u.complete});await gt(h),this.onShowFeedback&&this.onShowFeedback(h),console.log("Coach: Evaluation complete",h)}catch(c){const l=c.message||"Coach evaluation failed";f(l),console.error("Coach: Error",c),await ne(t.coachProvider||"openai",t.coachModel||"unknown",l,Math.round(performance.now()-d))}finally{this.isCoachRunning=!1,this.coachBtn.disabled=!1,this.setButtonLoading(this.coachBtn,!1)}}setButtonLoading(e,t){if(!document.getElementById("btn-spinner-styles")){const o=document.createElement("style");o.id="btn-spinner-styles",o.textContent=`
      ...
      `}),e.querySelectorAll("li").forEach(r=>{r.style.cssText="margin: 0.2em 0;"}),e.querySelectorAll("h1, h2, h3, h4, h5, h6").forEach(r=>{r.style.marginTop="1em",r.style.marginBottom="0.4em"})}}class ze{constructor(e,t={}){this.entry=e,this.element=null,this.isExpanded=!1,this.onTitleClick=t.onTitleClick||null}render(){this.element=document.createElement("div"),this.element.className="feedback-balloon",this.element.style.cssText=`
      padding: var(--pc-space-3);
    ...
    `;const s=document.createElement("div");s.style.cssText=`
      ...
      cursor: pointer;
      -webkit-tap-highlight-color: transparent;
    `,s.title="Click to view in popup";const i=document.createElement("div"),o=this.entry.overall,r=o>=80?"var(--pc-primary)":o>=60?"#F59E0B":"#EF4444";i.style.cssText=`
      min-width: 32px;
      width: 32px;
      height: 32px;
      flex-shrink: 0;
    ...
    `,a.textContent=this.entry.description,s.appendChild(a),s.addEventListener("click",()=>{this.onTitleClick&&this.onTitleClick(this.entry)}),t.appendChild(s);const d=document.createElement("span");d.style.cssText=`
      font-size: 0.75rem;
      color: var(--pc-on-surface-variant);
      flex-shrink: 0;
    ...
    `,c.textContent=this.entry.feedback,this.element.appendChild(c),this.entry.feedback.length>200){const l=document.createElement("button");l.style.cssText=`
      ...
      `,this.entry.scores.forEach(h=>{const g=document.createElement("span");g.style.cssText=`
          ...
          border-radius: var(--pc-radius-full);
          font-size: 0.75rem;
        `,g.innerHTML=`<span style="text-transform: capitalize;">${h.principle}</span>: <strong>${h.score}</strong>`,m.appendChild(g)}),l.appendChild(m),this.element.appendChild(l)}if(this.entry.promptSnapshot){const l=document.createElement("details");l.style.cssText="margin-top: var(--pc-space-2);";const u=document.createElement("summary");u.style.cssText=`
        cursor: pointer;
        font-size: 0.8rem;
        color: var(--pc-on-surface-variant);
        display: flex;
    ...
    `}appendChunk(e){this.pendingText+=e,this.rafId||(this.rafId=requestAnimationFrame(()=>{this.result.responseText=this.pendingText,this.responseEl&&(this.responseEl.innerHTML=ue(this.pendingText)||this.pendingText,this.applyMarkdownStyles()),this.rafId=null}))}complete(e,t){var i;this.result.status="complete",this.result.tokens=e||this.result.tokens,this.result.durationMs=t||this.result.durationMs,this.result.responseText=this.pendingText||this.result.responseText,this.updateResponse(),this.updateStats();const s=(i=this.element)==null?void 0:i.querySelector(".result-status");s&&this.updateStatusBadge(s)}setError(e,t){var i;this.result.status="error",this.result.error=e,this.result.durationMs=t||this.result.durationMs,this.pendingText&&(this.result.responseText=this.pendingText),this.updateResponse();const s=(i=this.element)==null?void 0:i.querySelector(".result-status");s&&this.updateStatusBadge(s)}toggle(){var t;this.expanded=!this.expanded,this.contentEl&&(this.contentEl.style.display=this.expanded?"block":"none");const e=(t=this.element)==null?void 0:t.querySelector(".chevron");e&&(e.innerHTML=this.expanded?'<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M7.41 8.59L12 13.17l4.59-4.58L18 10l-6 6-6-6 1.41-1.41z"/></svg>':'<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>'),this.onToggle&&this.onToggle(this.result.id,this.expanded)}expand(){this.expanded||this.toggle()}collapse(){this.expanded&&this.toggle()}async copyResponse(){const e=this.result.responseText;if(!e){f("Nothing to copy");return}try{await navigator.clipboard.writeText(e),f("Copied to clipboard")}catch{f("Failed to copy")}}formatTime(e){return new Date(e).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}getResult(){return this.result}}class ss{constructor(){this.element=null,this.headerEl=null,this.contentEl=null,this.emptyStateEl=null,this.balloons=new Map}render(){return this.element?this.element:(this.element=document.createElement("div"),this.element.className="results-tab",this.element.setAttribute("role","tabpanel"),this.element.setAttribute("id","results-panel"),this.element.style.cssText=`
      display: flex;
      flex-direction: column;
      height: 100%;
    ...
    `,s.addEventListener("click",()=>this.handleClear()),e.appendChild(s);const i=document.createElement("button");return i.className="btn btn-text-compact",i.title="Download results",i.innerHTML=`
      ...
      <span>Save</span>
    `,i.addEventListener("click",()=>this.handleDownload()),e.appendChild(i),e}async loadResults(){const e=await ke();if(e.length===0){this.showEmptyState();return}this.hideEmptyState(),this.balloons.clear(),this.contentEl.innerHTML="",this.contentEl.appendChild(this.emptyStateEl),this.emptyStateEl.style.display="none",[...e].reverse().forEach((s,i)=>{const o=new $e(s,{expanded:i===0,onToggle:(r,a)=>this.handleBalloonToggle(r,a)});this.balloons.set(s.id,o),this.contentEl.insertBefore(o.render(),this.emptyStateEl)}),this.updateSessionStats()}addResult(e){if(this.balloons.has(e.id))return this.balloons.get(e.id);this.hideEmptyState(),this.balloons.forEach(s=>s.collapse());const t=new $e(e,{expanded:!0,onToggle:(s,i)=>this.handleBalloonToggle(s,i)});return this.balloons.set(e.id,t),this.contentEl.insertBefore(t.render(),this.contentEl.firstChild),this.updateSessionStats(),t}getBalloon(e){return this.balloons.get(e)}handleBalloonToggle(e,t){}async handleClear(){this.balloons.size===0||!confirm("Clear all results? This cannot be undone.")||(await ut(),this.balloons.clear(),this.contentEl.innerHTML="",this.contentEl.appendChild(this.emptyStateEl),this.showEmptyState(),this.updateSessionStats(),f("Results cleared"))}async handleDownload(){const e=await ke();if(e.length===0){f("No results to download");return}const t=this.generateMarkdown(e),s=new Blob([t],{type:"text/markdown"}),i=URL.createObjectURL(s),o=document.createElement("a");o.href=i,o.download=`prompt-coach-results-${new Date().toISOString().split("T")[0]}.md`,document.body.appendChild(o),o.click(),document.body.removeChild(o),URL.revokeObjectURL(i),f("Results downloaded")}generateMarkdown(e){let t=`# Prompt Coach - Test Results
...
`}),t}async updateSessionStats(){const t=(await M()).totalTokens||0,s=this.balloons.size;this.statsEl&&(this.statsEl.textContent=`${s} result${s!==1?"s":""} • ${t.toLocaleString()} tokens`)}showEmptyState(){this.emptyStateEl&&(this.emptyStateEl.style.display="flex")}hideEmptyState(){this.emptyStateEl&&(this.emptyStateEl.style.display="none")}}class ns{constructor(){this.element=null,this.contentArea=null,this.currentTab=W().currentTab||"prompt",this.tabs={},this.menuPanel=null,this.feedbackPanel=null,this.ribbon=null}render(){this.element=document.createElement("div"),this.element.className="app-shell",this.element.style.cssText=`
    ...
    `,this.providerSelect=o,s.appendChild(i),s.appendChild(o),e.appendChild(s);const r=document.createElement("div");r.style.cssText="margin-bottom: var(--pc-space-4);";const a=document.createElement("label");a.className="text-label",a.style.cssText="display: block; margin-bottom: var(--pc-space-2); color: var(--pc-on-surface-variant);",a.textContent="API Key";const d=document.createElement("input");return d.type="password",d.className="input",d.placeholder="Enter your API key",d.autocomplete="off",this.keyInput=d,r.appendChild(a),r.appendChild(d),e.appendChild(r),e}show(){this.render(),this.setContent(this.createContent()),this.addAction("Skip",()=>this.close(),"text"),this.addAction("Save",()=>this.save(),"filled"),super.show(),setTimeout(()=>{this.keyInput.focus()},100)}save(){const e=this.providerSelect.value,t=this.keyInput.value.trim();if(!t){f("Please enter an API key");return}Ge("apiKeys",{[e]:t}),f("API key saved"),this.close()}}function rs(){const n=document.getElementById("loader");if(!n)return;n.style.pointerEvents="none";const e=2e3,t=2e3,s=performance.now(),i=m=>m===1?1:1-Math.pow(2,-10*m),o=Math.random()*Math.PI*2,r=Math.random()*Math.PI*2,a=.25+Math.random()*.125,d=.1875+Math.random()*.125,c=60,l=48;function u(m){const h=m-s,g=Math.min(h/e,1),p=Math.min(h/t,1),y=h/2e3*Math.PI*2,x=50+Math.sin(y*a+o)*c*(1-p),v=50+Math.cos(y*d+r)*l*(1-p),w=1-p,k=20*(1-i(g));n.style.backdropFilter=`blur(${k}px)`,n.style.webkitBackdropFilter=`blur(${k}px)`,n.style.background=`radial-gradient(
      ...
      rgba(28, 27, 31, ${w}) 100%
    )`,p<1?requestAnimationFrame(u):n.remove()}requestAnimationFrame(u)}async function Fe(){try{bt(),await T(),await M();const n=W(),e=document.getElementById("app"),t=new ns;e.appendChild(t.render()),rs(),n.firstRunCompleted||new is({onDismiss:()=>{I("firstRunCompleted",!0),oe()||new os().show()}}).show(),"serviceWorker"in navigator&&window.addEventListener("load",()=>{navigator.serviceWorker.register("./sw.js").catch(()=>{})}),Lt(),window.canInstallPWA=be}catch(n){console.error("Failed to initialize app:",n),document.getElementById("app").innerHTML=`
